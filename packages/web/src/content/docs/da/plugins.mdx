---
title: Plugins
description: Skriv dine egne plugins for at udvide Reimagined.
---

Plugins giver dig mulighed for at udvide Reimagined ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre Reimagineds standardadfærd.
...
- `.reimagined/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/reimagined/node_modules/`.
...
1. Global konfiguration (`~/.config/reimagined/reimagined.json`)
2. Projektkonfiguration (`reimagined.json`)
3. Global plugin-mappe (`~/.config/reimagined/plugins/`)
4. Projekt plugin bibliotek (`.reimagined/plugins/`)
...
Reimagined kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En reimagined SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide reimagined.
...
:::note
Hvis du bruger Reimagined desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå reimagined i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til reimagined:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som reimagined kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for reimagined sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://reimagined.ai/docs/sdk) for detaljer.
