...
description: Cree herramientas que LLM pueda llamar en reimagined.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de reimagined como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.reimagined/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/reimagined/tools/`.
...
```ts title=".reimagined/tools/database.ts" {1}
...
```ts title=".reimagined/tools/math.ts"
import { tool } from "@reimagined-ai/plugin"
...
```ts title=".reimagined/tools/bash.ts"
import { tool } from "@reimagined-ai/plugin"
...
```ts title=".reimagined/tools/project.ts" {8}
import { tool } from "@reimagined-ai/plugin"
...
```python title=".reimagined/tools/add.py"
import sys
...
```ts title=".reimagined/tools/python-add.ts" {10}
import { tool } from "@reimagined-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".reimagined/tools/add.py")
