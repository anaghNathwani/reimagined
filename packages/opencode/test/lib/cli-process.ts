// Subprocess test harness for the reimagined CLI. Spawns the real binary against
...
//
// Configuration flows through reimagined's built-in test affordances:
//   - OPENCODE_CONFIG_CONTENT      : provider config inline, no files to find
//   - OPENCODE_TEST_HOME           : pins os.homedir() → tmpdir
//   - OPENCODE_DISABLE_PROJECT_CONFIG : skip walking up for reimagined.json
...
//
// Today only `reimagined.run` is fully wired. The shape supports adding more
// builders (`reimagined.serve(opts)`, `reimagined.acp(opts)`, `reimagined.auth(...)`)
...
const reimaginedRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(reimaginedRoot, "src/index.ts")
...
// Typed equivalent of constructing argv for `reimagined run`. New flags should
...
// `reimagined serve` is a long-lived process — it never exits on its own.
...
// `reimagined acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type OpencodeCli = {
  ...
  // Spawn `reimagined serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `reimagined acp` and return a duplex JSON-RPC handle. Long-lived:
...
export type CliFixture = {
  ...
  readonly reimagined: OpencodeCli
}
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("reimagined.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const startRun = Effect.fn("reimagined.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("reimagined.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "reimagined server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `reimagined serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("reimagined.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const reimagined: OpencodeCli = { run, startRun, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, reimagined })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `reimagined.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
      Layer.mergeAll(
        TestLLMServer.layer,
...
function expectExit(result: RunResult, expected: number, label = "reimagined") {
...
// every call site. Use this for any test that needs the reimagined CLI fixture.
//
...
// resources (e.g. `reimagined.serve`) without an extra `Effect.scoped` wrapper —
