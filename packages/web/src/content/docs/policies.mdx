---
title: Policies
description: Control which configured resources Reimagined may use.
---

Policies control whether Reimagined may perform an action on a named resource. This feature is experimental and is configured with the `experimental.policies` array in `reimagined.json`.

Policies are separate from [permissions](/docs/permissions). Permissions control what tools can do during a session, while policies control whether Reimagined may use a resource such as an LLM provider.
...
Reimagined currently supports one policy action:
