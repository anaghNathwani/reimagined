...
reimagined ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak reimagined kendi `reimagined` temasını kullanır.
...
reimagined birden fazla yerleşik temayla gelir.
...
- reimagined'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
reimagined, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/reimagined/themes/*.json` veya `$XDG_CONFIG_HOME/reimagined/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.reimagined/themes/*.json`
4. **Geçerli çalışma dizini** - `./.reimagined/themes/*.json`
