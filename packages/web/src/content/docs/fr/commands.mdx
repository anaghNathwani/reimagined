...
Créez `.reimagined/commands/test.md` :
...
Vous pouvez ajouter des commandes personnalisées via la configuration Reimagined ou en créant des fichiers markdown dans le répertoire `commands/`.

---

### JSON

Utilisez l'option `command` dans votre Reimagined [config](/docs/config) :
...
- Par projet : `.reimagined/commands/`
...
reimagined comprend plusieurs commandes intégrées comme `/init`, `/undo`, `/redo`, `/share`, `/help` ; [en savoir plus](/docs/tui#commands).
