...
const directory = "C:/Reimagined/TerminalComposerFocus"
...
test.beforeEach(async ({ page }) => {
  await mockReimaginedServer(page, {
    ...
    provider: {
      ...
      default: { providerID: "reimagined", modelID: "test" },
    },
    sessions: [
      {
        id: sessionID,
...
function seedCachedTerminal(page: Page) {
  return page.addInitScript(
    ({ terminalKey, ptyID }) => {
      localStorage.setItem("reimagined.global.dat:layout", JSON.stringify({ terminal: { height: 320, opened: true } }))
