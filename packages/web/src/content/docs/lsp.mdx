---
title: LSP Servers
description: Reimagined integrates with your LSP servers.
---

Reimagined can integrate with Language Server Protocol (LSP) servers to use diagnostics as feedback for the agent.

---

## Built-in

Reimagined comes with several built-in LSP servers for popular languages:
...
When LSP is enabled and reimagined opens a file, it:
...
You can enable and customize LSP servers through the `lsp` section in your reimagined config.
...
```json title="reimagined.json" {5-8}
{
  "$schema": "https://reimagined.ai/config.json",
  "lsp": {
    "rust": {
...
```json title="reimagined.json" {5-13}
{
  "$schema": "https://reimagined.ai/config.json",
  "lsp": {
    "custom-lsp": {
