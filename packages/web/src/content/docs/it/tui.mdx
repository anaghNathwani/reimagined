---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di Reimagined.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Reimagined fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire Reimagined avvia la TUI per la directory corrente.
...
Quando usi la TUI di Reimagined, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a Reimagined. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da Reimagined. _Alias_: `/quit`, `/q`
...
Questo è separato da `reimagined.json`, che configura il comportamento di server/runtime.
...
- `leader_timeout` - Controlla per quanto tempo Reimagined attende dopo il leader key. Default: `2000`.
...
- `sound_pack` - ID del sound pack da usare. Default: `reimagined.default`.
