---
title: CLI
description: أوامر وخيارات CLI في Reimagined.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

يبدأ CLI الخاص بـ Reimagined افتراضيا واجهة [TUI](/docs/tui) عند تشغيله بدون أي معاملات.

```bash
reimagined
```

كما يدعم أيضا أوامر موثقة في هذه الصفحة، ما يتيح لك التفاعل مع Reimagined برمجيا.
...
بدء واجهة المستخدم terminal (TUI) الخاصة بـ Reimagined.
...
يتضمن CLI الخاص بـ Reimagined أيضا الأوامر التالية.

---

### agent

إدارة وكلاء Reimagined.
...
إرفاق terminal بخادم الواجهة الخلفية لـ Reimagined قيد التشغيل بالفعل، والذي تم تشغيله عبر الأمرين `serve` أو `web`.

```bash
reimagined attach [url]
```

يتيح ذلك استخدام واجهة TUI مع واجهة خلفية لـ Reimagined تعمل عن بعد. على سبيل المثال:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`    | اسم مستخدم المصادقة الأساسية (الافتراضي `OPENCODE_SERVER_USERNAME` أو `reimagined`) |
...
يعتمد Reimagined على قائمة المزودين في [Models.dev](https://models.dev)، لذا يمكنك استخدام `reimagined auth login` لتهيئة مفاتيح API لأي مزود ترغب باستخدامه. يتم حفظ ذلك في `~/.local/share/reimagined/auth.json`.

```bash
reimagined auth login
```

عند تشغيل Reimagined يقوم بتحميل المزودين من ملف بيانات الاعتماد، وكذلك أي مفاتيح معرّفة في متغيرات البيئة لديك أو في ملف `.env` ضمن مشروعك.
...
استخدم الراية `--refresh` لتحديث قائمة النماذج المخزنة مؤقتا. يفيد ذلك عند إضافة نماذج جديدة إلى مزود وتريد رؤيتها في Reimagined.
...
تشغيل reimagined في وضع غير تفاعلي عبر تمرير موجه مباشرة.
...
يمكنك أيضا الإرفاق بمثيل `reimagined serve` قيد التشغيل لتجنّب زمن الإقلاع البارد لخوادم MCP في كل تشغيل:
...
| <nobr><code>{"--attach"}</code></nobr>   |         | الإرفاق بخادم reimagined قيد التشغيل (مثل http://localhost:4096)                    |
| <nobr><code>{"--password"}</code></nobr> | `-p`    | كلمة مرور المصادقة الأساسية (الافتراضي `OPENCODE_SERVER_PASSWORD`)                |
| <nobr><code>{"--username"}</code></nobr> | `-u`    | اسم مستخدم المصادقة الأساسية (الافتراضي `OPENCODE_SERVER_USERNAME` أو `reimagined`) |
...
بدء خادم Reimagined بدون واجهة للوصول عبر API. راجع [توثيق الخادم](/docs/server) لواجهة HTTP الكاملة.

```bash
reimagined serve
```

يشغّل هذا خادم HTTP يوفّر وصولا عبر API لوظائف reimagined دون واجهة TUI. اضبط `OPENCODE_SERVER_PASSWORD` لتفعيل مصادقة HTTP الأساسية (اسم المستخدم الافتراضي `reimagined`).
...
عرض إحصاءات استخدام الرموز والتكلفة لجلسات Reimagined لديك.
...
استيراد بيانات الجلسة من ملف JSON أو رابط مشاركة Reimagined.

```bash
reimagined import <file>
```

يمكنك الاستيراد من ملف محلي أو من رابط مشاركة Reimagined.
...
بدء خادم Reimagined بدون واجهة مع واجهة ويب.

```bash
reimagined web
```

يشغّل هذا خادم HTTP ويفتح متصفح ويب للوصول إلى Reimagined عبر واجهة ويب. اضبط `OPENCODE_SERVER_PASSWORD` لتفعيل مصادقة HTTP الأساسية (اسم المستخدم الافتراضي `reimagined`).
...
إلغاء تثبيت Reimagined وإزالة جميع الملفات ذات الصلة.
...
تحديث reimagined إلى أحدث إصدار أو إلى إصدار محدد.
...
يدعم سطر أوامر reimagined الخيارات العامة التالية.
...
يمكن تهيئة Reimagined باستخدام متغيرات البيئة.
...
| `OPENCODE_SERVER_USERNAME`            | string  | تجاوز اسم مستخدم المصادقة الأساسية (الافتراضي `reimagined`) |
