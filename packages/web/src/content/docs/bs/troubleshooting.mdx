...
Da biste otklonili probleme s Reimagined, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\reimagined\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `reimagined --log-level DEBUG`.

---

## Pohrana

Reimagined pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/reimagined/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\reimagined`
...
Reimagined Desktop pokreće lokalni Reimagined server (`reimagined-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `Reimagined` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/reimagined/reimagined.jsonc` (ili `~/.config/reimagined/reimagined.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/reimagined/reimagined.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\reimagined\reimagined.jsonc`
...
Reimagined također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/reimagined/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\reimagined\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga Reimagined mogao ponovo izgraditi.

1. Potpuno zatvorite Reimagined Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/reimagined`
- **Linux**: obrišite `~/.cache/reimagined` (ili pokrenite `rm -rf ~/.cache/reimagined`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\reimagined`

3. Ponovo pokrenite Reimagined Desktop.

---

### Rješavanje problema sa vezom na serveru

Reimagined Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `reimagined.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.
...
Na Windows-u, Reimagined Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa Reimagined karakteristikama.

---

### Obavještenja se ne prikazuju

Reimagined Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za Reimagined u postavkama vašeg OS-a, i
...
1. Zatvorite Reimagined Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka Reimagined Desktop aplikacije):

- `reimagined.settings.dat` (URL zadanog servera za desktop)
- `reimagined.global.dat` i `reimagined.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Ako imate problema s Reimagined:
...
Da saznate kojim modelima imate pristup, pokrenite `reimagined models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\reimagined`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. Reimagined dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\reimagined`

2. Ponovo pokrenite Reimagined da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti Reimagined da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
Reimagined će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
