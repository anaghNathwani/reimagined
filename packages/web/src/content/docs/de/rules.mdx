---
title: Regeln
description: Lege eigene Anweisungen fuer reimagined fest.
---

Du kannst reimagined ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in reimagined den Befehl `/init` aus.
...
So kann reimagined sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht reimagined sie zu erweitern.
...
reimagined kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/reimagined/AGENTS.md` speichern.
Diese gelten in allen reimagined-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt Reimagined dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/reimagined/AGENTS.md` existiert)
...
Beim Start sucht reimagined in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/reimagined/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `reimagined.json` oder in `~/.config/reimagined/reimagined.json` angeben.
...
reimagined parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit reimagined.json

Empfohlen ist das `instructions`-Feld in der `reimagined.json`:
...
Du kannst reimagined explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `reimagined.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
