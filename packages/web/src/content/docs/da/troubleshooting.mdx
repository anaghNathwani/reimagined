...
For at fejlfinde problemer med Reimagined, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\reimagined\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `reimagined --log-level DEBUG`.

---

## Lagring

reimagined gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/reimagined/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\reimagined`
...
Reimagined Desktop kører en lokal Reimagined-server (`reimagined-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `Reimagined`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/reimagined/reimagined.jsonc` (eller `~/.config/reimagined/reimagined.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/reimagined/reimagined.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\reimagined\reimagined.jsonc`
...
Reimagined kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/reimagined/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\reimagined\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så Reimagined kan genopbygge den.

1. Afslut Reimagined Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/reimagined`
- **Linux**: slet `~/.cache/reimagined` (eller kør `rm -rf ~/.cache/reimagined`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\reimagined`

3. Start Reimagined Desktop på ny.

---

### Løs problemer med serverforbindelse

Reimagined Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `reimagined.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.
...
På Windows kræver Reimagined Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i Reimagined.

---

### Meddelelser vises ikke

Reimagined Desktop viser kun systemvarsler når:

- varsler er aktiveret for Reimagined i dine OS-indstillinger, og
...
1. Afslut Reimagined Desktop.
2. Find og slet disse filer (de findes i Reimagined Desktop-appens datamappe):

- `reimagined.settings.dat` (skrivebordsstandardserver URL)
- `reimagined.global.dat` og `reimagined.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med Reimagined:
...
For at finde ud af hvilke modeller du har adgang til, kør `reimagined models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\reimagined`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. reimagined installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\reimagined`

2. Start reimagined på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge reimagined til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
reimagined vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
