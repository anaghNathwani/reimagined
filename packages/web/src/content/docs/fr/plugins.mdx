---
title: Plugins
description: Écrivez vos propres plugins pour étendre Reimagined.
---

Les plugins vous permettent d'étendre Reimagined en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de Reimagined.
...
- `.reimagined/plugins/` - Plugins au niveau du projet
- `~/.config/reimagined/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/reimagined/node_modules/`.
...
1. Configuration globale (`~/.config/reimagined/reimagined.json`)
2. Configuration du projet (`reimagined.json`)
3. Répertoire global des plugins (`~/.config/reimagined/plugins/`)
4. Répertoire des plugins du projet (`.reimagined/plugins/`)
...
Reimagined exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK reimagined pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre reimagined.
...
:::note
Si vous utilisez l'application de bureau Reimagined, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez reimagined de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à reimagined :
...
L'assistant `tool` crée un outil personnalisé que reimagined peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour reimagined aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://reimagined.ai/docs/sdk) pour plus de détails.
