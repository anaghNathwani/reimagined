...
Možete dodati vanjske alate u Reimagined koristeći _Model Context Protocol_, ili MCP. Reimagined podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [Reimagined Config](https://reimagined.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/reimagined`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
Reimagined automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, Reimagined će:
...
Ako server zahtijeva autentifikaciju, Reimagined će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `reimagined mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, Reimagined sigurno čuva tokene u `~/.local/share/reimagined/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u Reimagined, zajedno s ugrađenim alatima. Možete njima upravljati kroz Reimagined konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete Reimagined sa vašim Sentry nalogom.
