---
title: Servidor
description: Interactuar con el servidor reimagined a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `reimagined serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente reimagined puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `reimagined.local` |
...
Configure `OPENCODE_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `reimagined`, o configure `OPENCODE_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `reimagined serve` como a `reimagined web`.
...
Cuando ejecuta `reimagined`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor reimagined para interactuar con reimagined mediante programación.
:::

Esta arquitectura permite que reimagined admita múltiples clientes y le permite interactuar con reimagined mediante programación.

Puede ejecutar `reimagined serve` para iniciar un servidor independiente. Si tienes el
reimagined TUI ejecutándose, `reimagined serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos Reimagined [IDE](/docs/ide).
...
El servidor reimagined expone las siguientes API.
