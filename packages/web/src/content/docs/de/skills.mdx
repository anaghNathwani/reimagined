...
Agent Skills erlauben Reimagined, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
Reimagined sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft Reimagined vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.reimagined/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/reimagined/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.reimagined/skills/git-release/SKILL.md` zum Beispiel so:
...
Reimagined listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `reimagined.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `reimagined.json`):
