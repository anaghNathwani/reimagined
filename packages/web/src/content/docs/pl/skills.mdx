...
Umiejętności agenta pozwalają reimagined odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
reimagined przeszukuje te lokalizacje:

- Project config: `.reimagined/skills/<name>/SKILL.md`
- Global config: `~/.config/reimagined/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu reimagined przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.reimagined/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/reimagined/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.reimagined/skills/git-release/SKILL.md` w ten sposób:
...
reimagined wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `reimagined.json`:
...
**Dla agentów wbudowanych** (w `reimagined.json`):
