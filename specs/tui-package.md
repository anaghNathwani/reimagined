...
Move the canonical Reimagined terminal application from
`packages/reimagined/src/cli/cmd/tui` into a self-contained workspace package while
...
```text
packages/reimagined ---\
                      > @reimagined-ai/tui -> @reimagined-ai/sdk
packages/cli --------/
```
...
generic presentation libraries. It must not depend on `packages/reimagined`,
`packages/cli`, or `@reimagined-ai/core`.

The SDK is the TUI's Reimagined boundary. Missing backend data or operations must
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/reimagined`.
- Do not replace private `packages/reimagined` imports with `@reimagined-ai/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@reimagined-ai/tui`.

## Ownership Boundary

### `@reimagined-ai/tui` Owns
...
- Reimagined domain data displayed by the TUI
...
```text
packages/reimagined/src/cli/cmd/tui
```

Its private dependency on `packages/reimagined` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/reimagined/src/*`.
...
Status: Completed. The private `@reimagined-ai/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@reimagined-ai/tui`.
...
- The package has no dependency on `reimagined`, `@reimagined-ai/cli`, or
  `@reimagined-ai/core`.
...
primitives now live in `@reimagined-ai/tui`. The legacy host consumes explicit
...
and build inputs through `@reimagined-ai/tui/runtime`. Movable app, component,
route, and feature-plugin code no longer reads Reimagined globals or process state;
...
Replace process-global Reimagined state with resolved TUI inputs.
...
`@reimagined-ai/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/reimagined` initially.
...
canonical in `@reimagined-ai/tui`. Configured references resolve through the new
...
adapters rather than Reimagined domain implementations.

Make the SDK the only Reimagined domain boundary used by the TUI.
...
- Domain-facing TUI code imports Reimagined data and operations only from
  `@reimagined-ai/sdk`.
...
`@reimagined-ai/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/reimagined`.
...
- The package has no imports from `packages/reimagined`, `packages/cli`, or
  `@reimagined-ai/core`.
...
public `@reimagined-ai/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/reimagined`.
- Keep the legacy embedded worker and server startup in `packages/reimagined`.
- Change those adapters to load config, create transport inputs, and call the
  public `@reimagined-ai/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/reimagined`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@reimagined-ai/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@reimagined-ai/core`, or either
  executable package.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for Reimagined domain state.
...
cd packages/reimagined && bun typecheck
...
rg '@reimagined-ai/core|packages/reimagined|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/reimagined/src packages/reimagined/test
```
...
- Build the current-platform `packages/reimagined` binary.
