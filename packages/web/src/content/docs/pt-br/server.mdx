---
title: Servidor
description: Interaja com o servidor reimagined via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `reimagined serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente reimagined pode usar.
...
```bash
reimagined serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `reimagined.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
reimagined serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `OPENCODE_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `reimagined`, ou defina `OPENCODE_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `reimagined serve` quanto ao `reimagined web`.

```bash
OPENCODE_SERVER_PASSWORD=your-password reimagined serve
```
...
Quando você executa `reimagined`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor reimagined para interagir com o reimagined programaticamente.
:::

Essa arquitetura permite que o reimagined suporte múltiplos clientes e permite que você interaja com o reimagined programaticamente.

Você pode executar `reimagined serve` para iniciar um servidor autônomo. Se você tiver o TUI do reimagined em execução, `reimagined serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do reimagined [IDE](/docs/ide).
...
O servidor reimagined expõe as seguintes APIs.
