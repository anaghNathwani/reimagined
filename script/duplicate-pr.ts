...
import { createOpencode } from "@reimagined-ai/sdk"
...
`)
  ...
  const reimagined = await createOpencode({ port: 0 })
  ...
  try {
    ...
    const session = await reimagined.client.session.create()
    const result = await reimagined.client.session
      .prompt({
