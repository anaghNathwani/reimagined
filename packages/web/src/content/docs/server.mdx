---
title: Server
description: Interact with reimagined server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `reimagined serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an reimagined client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `reimagined.local` |
...
Set `OPENCODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `reimagined`, or set `OPENCODE_SERVER_USERNAME` to override it. This applies to both `reimagined serve` and `reimagined web`.
...
When you run `reimagined` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the reimagined server to interact with reimagined programmatically.
:::

This architecture lets reimagined support multiple clients and allows you to interact with reimagined programmatically.

You can run `reimagined serve` to start a standalone server. If you have the
reimagined TUI running, `reimagined serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Reimagined [IDE](/docs/ide) plugins.
...
The reimagined server exposes the following APIs.
