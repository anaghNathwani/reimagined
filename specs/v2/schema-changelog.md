...
This document covers meaningful contract changes introduced on the `feat/reimagined-embedded-api` branch since its divergence from `origin/dev`. Mechanical file moves and internal refactors are omitted unless they changed stored data, replay behavior, public HTTP or SDK shapes, or model-facing tool contracts.
...
- Canonical tagged tool file sources in `@reimagined-ai/llm`.
...
- Spill oversized model-facing tool text into globally unique files under Reimagined's shared tool-output directory.
