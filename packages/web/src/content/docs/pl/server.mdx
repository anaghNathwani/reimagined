---
title: Serwer
description: Interakcja z serwerem reimagined za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `reimagined serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient reimagined.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `reimagined.local` |
...
Ustaw `OPENCODE_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `reimagined` lub ustaw `OPENCODE_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `reimagined serve`, jak i `reimagined web`.
...
Kiedy uruchomisz `reimagined`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera reimagined do programowej interakcji z reimagined.
:::

Ta architektura umożliwia obsługę wielu klientów przez reimagined i programową interakcję z kodem otwartym.

Możesz uruchomić `reimagined serve`, aby uruchomić samodzielny serwer. Jeśli masz
reimagined TUI działa, `reimagined serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki reimagined [IDE](/docs/ide).
...
Serwer reimagined udostępnia następujące interfejsy API.
