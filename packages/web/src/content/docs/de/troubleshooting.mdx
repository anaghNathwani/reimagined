...
Wenn Reimagined Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\reimagined\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `reimagined --log-level DEBUG`.

---

## Speicher

reimagined speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/reimagined/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\reimagined`
...
Reimagined Desktop startet im Hintergrund einen lokalen Reimagined-Server (`reimagined-cli`-Sidecar).
...
- Nur macOS: `Reimagined`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/reimagined/reimagined.jsonc` (oder `~/.config/reimagined/reimagined.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/reimagined/reimagined.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\reimagined\reimagined.jsonc`
...
Reimagined kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/reimagined/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\reimagined\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit Reimagined ihn neu aufbauen kann.

1. Beende Reimagined Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/reimagined`
- **Linux**: loesche `~/.cache/reimagined` (oder fuehre aus: `rm -rf ~/.cache/reimagined`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\reimagined`

3. Starte Reimagined Desktop neu.

---

### Server-Verbindungsprobleme beheben

Reimagined Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `reimagined.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.
...
Unter Windows benoetigt Reimagined Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit Reimagined-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

Reimagined Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer Reimagined in den OS-Einstellungen aktiviert sind, und
...
1. Beende Reimagined Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von Reimagined Desktop):

- `reimagined.settings.dat` (desktop default server URL)
- `reimagined.global.dat` und `reimagined.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit Reimagined hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `reimagined models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\reimagined`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. reimagined installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\reimagined`

2. Starte reimagined neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt reimagined, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
reimagined erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
