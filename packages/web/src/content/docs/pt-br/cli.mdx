---
title: CLI
description: Opções e comandos da CLI do reimagined.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

A CLI do reimagined, por padrão, inicia o [TUI](/docs/tui) quando executada sem argumentos.

```bash
reimagined
```

Mas também aceita comandos conforme documentado nesta página. Isso permite que você interaja com o reimagined programaticamente.
...
Inicie a interface de usuário do terminal do reimagined.
...
A CLI do reimagined também possui os seguintes comandos.

---

### agent

Gerencie agentes para o reimagined.
...
Anexe um terminal a um servidor backend do reimagined já em execução, iniciado via comandos `serve` ou `web`.

```bash
reimagined attach [url]
```

Isso permite usar o TUI com um backend reimagined remoto. Por exemplo:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuário de autenticação básica (padrão: `OPENCODE_SERVER_USERNAME` ou `reimagined`) |
...
O reimagined é alimentado pela lista de provedores em [Models.dev](https://models.dev), então você pode usar `reimagined auth login` para configurar chaves de API para qualquer provedor que você gostaria de usar. Isso é armazenado em `~/.local/share/reimagined/auth.json`.

```bash
reimagined auth login
```

Quando o reimagined é iniciado, ele carrega os provedores do arquivo de credenciais. E se houver chaves definidas em seus ambientes ou em um arquivo `.env` em seu projeto.
...
Use a flag `--refresh` para atualizar a lista de modelos em cache. Isso é útil quando novos modelos foram adicionados a um provedor e você deseja vê-los no reimagined.
...
Execute o reimagined em modo não interativo passando um prompt diretamente.
...
```bash "reimagined run"
reimagined run Explique o uso de context em Go
```

Você também pode se anexar a uma instância em execução do `reimagined serve` para evitar tempos de inicialização a frio do servidor MCP em cada execução:
...
reimagined run --attach http://localhost:4096 "Explique async/await em JavaScript"
```
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Anexe a um servidor reimagined em execução (por exemplo, http://localhost:4096)     |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Senha de autenticação básica (padrão: `OPENCODE_SERVER_PASSWORD`)                 |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuário de autenticação básica (padrão: `OPENCODE_SERVER_USERNAME` ou `reimagined`) |
...
Inicie um servidor reimagined sem cabeça para acesso à API. Confira a [documentação do servidor](/docs/server) para a interface HTTP completa.

```bash
reimagined serve
```

Isso inicia um servidor HTTP que fornece acesso à funcionalidade do reimagined sem a interface TUI. Defina `OPENCODE_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `reimagined`).
...
Gerencie sessões do reimagined.
...
Liste todas as sessões do reimagined.
...
Mostre o uso de tokens e estatísticas de custo para suas sessões do reimagined.
...
Importe dados da sessão de um arquivo JSON ou URL de compartilhamento do reimagined.

```bash
reimagined import <file>
```

Você pode importar de um arquivo local ou de uma URL de compartilhamento do reimagined.
...
Inicie um servidor reimagined sem cabeça com uma interface web.

```bash
reimagined web
```

Isso inicia um servidor HTTP e abre um navegador para acessar o reimagined através de uma interface web. Defina `OPENCODE_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `reimagined`).
...
Desinstale o reimagined e remova todos os arquivos relacionados.
...
Atualiza o reimagined para a versão mais recente ou uma versão específica.
...
A CLI do reimagined aceita as seguintes flags globais.
...
O reimagined pode ser configurado usando variáveis de ambiente.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Substituir nome de usuário de autenticação básica (padrão `reimagined`) |
