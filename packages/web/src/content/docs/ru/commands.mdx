...
Создайте `.reimagined/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию reimagined или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем reimagined [config](/docs/config):
...
- Глобальный: `~/.config/reimagined/commands/`
- Для каждого проекта: `.reimagined/commands/`
...
reimagined включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
