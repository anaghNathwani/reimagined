...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.reimagined.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `reimagined.settings.dat` is special: it maps to the `reimagined.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"reimagined.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // reimagined.settings.dat → the electron settings store ("reimagined.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "reimagined.global.dat").
  const storeName = filename === "reimagined.settings.dat" ? "reimagined.settings" : filename
