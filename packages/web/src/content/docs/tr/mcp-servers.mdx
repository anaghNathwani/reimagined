...
_Model Bağlam Protokolü_ veya MCP kullanarak reimagined'a harici araçlar ekleyebilirsiniz. reimagined hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [reimagined Config](https://reimagined.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/reimagined` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
reimagined, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde reimagined şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa reimagined, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `reimagined mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından reimagined, belirteçleri `~/.local/share/reimagined/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra reimagined'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi reimagined yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve reimagined'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
