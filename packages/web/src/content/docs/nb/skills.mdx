...
Agentferdigheter lar Reimagined oppdage gjenbrukbare instruksjoner fra repo- eller hjemmekatalogen din.
...
reimagined søker etter disse stedene:

- Prosjektkonfigurasjon: `.reimagined/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/reimagined/skills/<name>/SKILL.md`
...
For prosjektlokale stier går reimagined opp fra din nåværende arbeidskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.reimagined/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/reimagined/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.reimagined/skills/git-release/SKILL.md` slik:
...
reimagined viser tilgjengelige ferdigheter i `skill`-verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved å bruke mønsterbaserte tillatelser i `reimagined.json`:
...
**For innebygde agenter** (i `reimagined.json`):
