---
title: CLI
description: Opzioni e comandi della CLI di Reimagined.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI di Reimagined, per impostazione predefinita, avvia la [TUI](/docs/tui) quando viene eseguita senza argomenti.

```bash
reimagined
```

Ma accetta anche i comandi documentati in questa pagina. Questo ti permette di interagire con Reimagined in modo programmatico.
...
Avvia l'interfaccia testuale di Reimagined (interfaccia utente da terminale).
...
La CLI di Reimagined include anche i seguenti comandi.

---

### agent

Gestisci gli agenti per Reimagined.
...
Collega un terminale a un backend server di Reimagined già in esecuzione avviato tramite i comandi `serve` o `web`.

```bash
reimagined attach [url]
```

Questo consente di usare la TUI con un backend Reimagined remoto. Per esempio:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nome utente per l'autenticazione di base (predefinito: `OPENCODE_SERVER_USERNAME` o `reimagined`) |
...
Reimagined si basa sull'elenco provider di [Models.dev](https://models.dev), quindi puoi usare `reimagined auth login` per configurare le API key per qualunque provider tu voglia usare. Le credenziali vengono salvate in `~/.local/share/reimagined/auth.json`.

```bash
reimagined auth login
```

Quando Reimagined si avvia, carica i provider dal file delle credenziali e, se presenti, anche eventuali key definite nell'ambiente o in un file `.env` nel progetto.
...
Usa `--refresh` per aggiornare l'elenco modelli in cache. È utile quando nuovi modelli vengono aggiunti a un provider e vuoi vederli in Reimagined.
...
Esegui reimagined in modalità non interattiva passando un prompt direttamente.
...
Puoi anche collegarti a una istanza `reimagined serve` già in esecuzione per evitare i cold boot dei server MCP ad ogni esecuzione:
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Attach a un server reimagined in esecuzione (es. http://localhost:4096)                           |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Password per l'autenticazione di base (predefinita: `OPENCODE_SERVER_PASSWORD`)                 |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nome utente per l'autenticazione di base (predefinito: `OPENCODE_SERVER_USERNAME` o `reimagined`) |
...
Avvia un server Reimagined headless per accesso via API. Vedi le [server docs](/docs/server) per l'interfaccia HTTP completa.

```bash
reimagined serve
```

Avvia un server HTTP che espone accesso API alle funzionalità di reimagined senza la TUI. Imposta `OPENCODE_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `reimagined`).
...
Gestisci le sessioni Reimagined.
...
Elenca tutte le sessioni Reimagined.
...
Mostra statistiche di utilizzo token e costo per le sessioni Reimagined.
...
Importa i dati di sessione da un file JSON o da un URL di condivisione Reimagined.

```bash
reimagined import <file>
```

Puoi importare da un file locale o da un URL di condivisione Reimagined.
...
Avvia un server Reimagined headless con interfaccia web.

```bash
reimagined web
```

Avvia un server HTTP e apre un browser per accedere a Reimagined tramite interfaccia web. Imposta `OPENCODE_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `reimagined`).
...
Disinstalla Reimagined e rimuove tutti i file correlati.
...
Aggiorna reimagined all'ultima versione o a una versione specifica.
...
La CLI di reimagined accetta i seguenti flag globali.
...
Reimagined può essere configurato tramite variabili d'ambiente.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Sovrascrive lo username basic auth (default `reimagined`)     |
