// Subprocess integration tests for `reimagined run` (non-interactive mode).
// These exercise the real CLI binary against a TestLLMServer running in the
// same process. See `test/lib/cli-process.ts` for the harness — each test uses
// `reimagined.run(message, opts?)` to spawn `bun src/index.ts run ...` with
...
describe("reimagined run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, reimagined }) =>
      Effect.gen(function* () {
        ...
        const result = yield* reimagined.run("say hi")
        reimagined.expectExit(result, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, reimagined }) =>
      Effect.gen(function* () {
        ...
        const result = yield* reimagined.run("use a tool", {
  ...
  cliIt.concurrent(
    ...
    ({ llm, reimagined }) =>
      Effect.gen(function* () {
        ...
        const thinking = yield* reimagined.run("think", { extraArgs: ["--thinking"] })
        reimagined.expectExit(thinking, 0)
        expect(thinking.stdout).toBe("Thinking: considering\nanswer\n")

        yield* llm.reason("hidden", { text: "visible" })
        const plain = yield* reimagined.run("think again")
        reimagined.expectExit(plain, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, reimagined }) =>
      Effect.gen(function* () {
        ...
        const result = yield* reimagined.run("trigger midstream error", { timeoutMs: 30_000 })
  ...
  cliIt.concurrent(
    ...
    ({ llm, reimagined }) =>
      Effect.gen(function* () {
        ...
        const result = yield* reimagined.run("say hi", { format: "json" })
        reimagined.expectExit(result, 0)

        const events = reimagined.parseJsonEvents(result.stdout)
  ...
  cliIt.concurrent(
    ...
    ({ reimagined }) =>
      Effect.gen(function* () {
        const result = yield* reimagined.run("use an unknown model", {
          model: "test/nonexistent-model",
          format: "json",
        })
  ...
  cliIt.concurrent(
    ...
    ({ llm, reimagined }) =>
      Effect.gen(function* () {
        ...
        const result = yield* reimagined.run("exercise json records", {
          format: "json",
  ...
  cliIt.concurrent(
    ...
    ({ llm, reimagined }) =>
      Effect.gen(function* () {
        ...
        const result = yield* reimagined.run("fail after output", { format: "json" })
  ...
  cliIt.concurrent(
    ...
    ({ home, llm, reimagined }) =>
      Effect.gen(function* () {
        ...
        const denied = yield* reimagined.run("request permission", { permission: { bash: "ask" } })
        reimagined.expectExit(denied, 0)
        ...
        const allowed = yield* reimagined.run("request permission", {
          permission: { bash: "ask" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        reimagined.expectExit(allowed, 0)
        ...
        const explicitlyDenied = yield* reimagined.run("request denied permission", {
          permission: { bash: "deny" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        reimagined.expectExit(explicitlyDenied, 0)
  ...
  cliIt.live(
    ...
    ({ home, llm, reimagined }) =>
      Effect.gen(function* () {
        ...
        const server = yield* reimagined.serve()

        const result = yield* reimagined.run("read the attachment", {
  ...
  cliIt.concurrent(
    ...
    ({ home, reimagined }) =>
      Effect.gen(function* () {
        const result = yield* reimagined.run("read the directory", {
  ...
  cliIt.live(
    ...
    ({ llm, reimagined }) =>
      Effect.gen(function* () {
        yield* llm.hang
        const run = yield* reimagined.startRun("wait forever")
        yield* llm.wait(1)
        run.interrupt()
