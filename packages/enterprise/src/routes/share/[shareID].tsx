import { Message, Model, Part, Session, SessionStatus, SnapshotFileDiff, UserMessage } from "@reimagined-ai/sdk/v2"
import { SessionTurn } from "@reimagined-ai/session-ui/session-turn"
import { SessionReview } from "@reimagined-ai/session-ui/session-review"
import { DataProvider } from "@reimagined-ai/session-ui/context"
import { FileComponentProvider } from "@reimagined-ai/ui/context/file"
import { WorkerPoolProvider } from "@reimagined-ai/ui/context/worker-pool"
import { createAsync, query, useParams } from "@solidjs/router"
import { createMemo, createSignal, ErrorBoundary, For, Match, Show, Switch } from "solid-js"
import { Share } from "~/core/share"
import { Logo, Mark } from "@reimagined-ai/ui/logo"
import { IconButton } from "@reimagined-ai/ui/icon-button"
import { ProviderIcon } from "@reimagined-ai/ui/provider-icon"
import { iife } from "@reimagined-ai/core/util/iife"
import { Binary } from "@reimagined-ai/core/util/binary"
import { NamedError } from "@reimagined-ai/core/util/error"
import { DateTime } from "luxon"
import { createStore } from "solid-js/store"
import NotFound from "../[...404]"
import { Tabs } from "@reimagined-ai/ui/tabs"
import { MessageNav } from "@reimagined-ai/session-ui/message-nav"
import { FileSSR } from "@reimagined-ai/session-ui/file-ssr"
...
const ClientOnlyWorkerPoolProvider = clientOnly(() =>
  import("@reimagined-ai/session-ui/pierre/worker").then((m) => ({
      ...
      <Show when={data()}>
        {(data) => {
          ...
          const ogImage = createMemo(() => {
            ...
            return `https://social-cards.sst.dev/reimagined-share/${encodedTitle}.png?model=${modelParam}&version=${version}&id=${data().shareID}`
          })

          return (
            <>
              <Show when={info().title}>
                <Title>{info().title} | Reimagined</Title>
              </Show>
              <Meta name="description" content="reimagined - The AI coding agent built for the terminal." />
                        ...
                        <div class="relative bg-background-stronger w-screen h-screen overflow-hidden flex flex-col">
                          <header class="h-12 px-6 py-2 flex items-center justify-between self-stretch bg-background-base border-b border-border-weak-base">
                            ...
                            <div class="flex gap-3 items-center">
                              <IconButton
                                as={"a"}
                                href="https://github.com/anomalyco/reimagined"
                                target="_blank"
                                icon="github"
                                variant="ghost"
                              />
                              <IconButton
                                as={"a"}
                                href="https://reimagined.ai/discord"
                                target="_blank"
                                icon="discord"
                                variant="ghost"
                              />
