...
With Reimagined you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Reimagined uses our own `reimagined` theme.
...
Reimagined comes with several built-in themes.
...
- Want Reimagined to match their terminal's appearance
...
Reimagined supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/reimagined/themes/*.json` or `$XDG_CONFIG_HOME/reimagined/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.reimagined/themes/*.json`
4. **Current working directory** - Defined in `./.reimagined/themes/*.json`
