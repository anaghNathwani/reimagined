...
Con Reimagined puoi scegliere tra diversi temi integrati, usare un tema che si adatta al tema del tuo terminale oppure definire un tema personalizzato.

Per impostazione predefinita, Reimagined usa il tema `reimagined`.
...
Reimagined include diversi temi integrati.
...
- Vuole che Reimagined corrisponda all'aspetto del terminale
...
Reimagined supporta un sistema di temi flessibile basato su JSON che permette di creare e personalizzare temi facilmente.
...
2. **Directory di configurazione utente** - in `~/.config/reimagined/themes/*.json` o `$XDG_CONFIG_HOME/reimagined/themes/*.json`
3. **Directory root del progetto** - in `<project-root>/.reimagined/themes/*.json`
4. **Directory di lavoro corrente** - in `./.reimagined/themes/*.json`
