...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "reimagined.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "reimagined.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "reimagined.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "reimagined.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["reimagined.json", "reimagined.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "reimagined.json"),
        path.join(project, "reimagined.jsonc"),
      ])
    })
  })
