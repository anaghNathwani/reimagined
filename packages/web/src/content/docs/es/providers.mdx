---
title: Proveedores
description: Usando cualquier proveedor LLM en Reimagined.
---

import config from "../../../../config.mjs"
export const console = config.console

Reimagined usa el [AI SDK](https://ai-sdk.dev/) y [Models.dev](https://models.dev) para admitir **75+ proveedores LLM** y admite la ejecución de modelos locales.

Para agregar un proveedor necesita:

1. Agregue las claves API para el proveedor usando el comando `/connect`.
2. Configure el proveedor en su configuración Reimagined.
...
Puedes personalizar los proveedores a través de la sección `provider` en tu Reimagined
configuración.
...
Reimagined Zen es una lista de modelos proporcionados por el equipo Reimagined que han sido
probado y verificado para funcionar bien con Reimagined. [Más información](/docs/zen).

:::tip
Si eres nuevo, te recomendamos comenzar con Reimagined Zen.
:::

1. Ejecute el comando `/connect` en TUI, seleccione reimagined y diríjase a [reimagined.ai/auth](https://reimagined.ai/auth).
...
Funciona como cualquier otro proveedor en Reimagined y su uso es completamente opcional.

---

## Reimagined Go

Reimagined Go es un plan de suscripción de bajo costo que brinda acceso confiable a modelos de codificación abiertos populares proporcionados por el equipo de Reimagined que han sido
probado y verificado para funcionar bien con Reimagined.

1. Ejecute el comando `/connect` en TUI, seleccione `Reimagined Go` y diríjase a [reimagined.ai/auth](https://reimagined.ai/zen).
...
Para usar Amazon Bedrock con Reimagined:
...
2. **Configure la autenticación** utilizando uno de los siguientes métodos:
   ...
   Establezca una de estas variables de entorno mientras ejecuta reimagined:
   ...
   Para una configuración persistente o específica del proyecto, utilice `reimagined.json`:
...
:::info
El uso de su suscripción Claude Pro/Max en Reimagined no está oficialmente respaldado por [Anthropic](https://anthropic.com).
:::
...
Puedes configurar reimagined para usar modelos locales mediante [Atomic Chat](https://atomic.chat), una aplicación de escritorio que ejecuta LLMs locales detrás de un servidor API compatible con OpenAI (endpoint por defecto `http://127.0.0.1:1337/v1`).
...
2. Vaya a [Azure AI Foundry](https://ai.azure.com/) e implemente un modelo.
   ...
   :::nota
   El nombre de la implementación debe coincidir con el nombre del modelo para que reimagined funcione correctamente.
   :::
...
5. Ejecute el comando `/models` para seleccionar un modelo.
   ...
   También puede agregar modelos a través de su configuración reimagined.
...
2. Elija su método de autenticación:
   ...
   3. Nombre: `Reimagined`, Alcances: `api`
...
para almacenar el token en el almacenamiento de autenticación reimagined.
:::

##### Autohospedado GitLab

:::note[nota de cumplimiento]
Reimagined utiliza un modelo pequeño para algunas tareas de IA, como generar el título de la sesión.
Está configurado para usar gpt-5-nano de forma predeterminada, alojado por Zen. Para bloquear Reimagined
Para usar solo su propia instancia alojada en GitLab, agregue lo siguiente a su
Archivo `reimagined.json`. También se recomienda desactivar el uso compartido de sesiones.
...
Más documentación en la página de inicio de [reimagined-gitlab-auth](https://www.npmjs.com/package/reimagined-gitlab-auth).

##### Configuración

Personalizar a través de `reimagined.json`:
...
Para utilizar su suscripción GitHub Copilot con reimagined:
...
Para utilizar Google Vertex AI con Reimagined:
...
2. Establezca las variables de entorno requeridas:
   ...
   Configúrelos mientras ejecuta reimagined.
...
En caso de que vea una característica o modelo de Helicone que no esté configurado automáticamente a través de reimagined, siempre podrá configurarlo usted mismo.
...
El complemento inyecta encabezados `Helicone-Session-Id` y `Helicone-Session-Name` en sus solicitudes. En la página Sesiones de Helicone, verá cada conversación Reimagined enumerada como una sesión separada.
...
Puede configurar reimagined para usar modelos locales a través de la utilidad llama-server de [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puede configurar reimagined para usar modelos locales a través de LM Studio.
...
Puedes configurar reimagined para usar modelos locales a través de Ollama.

:::tip
Ollama puede configurarse automáticamente para Reimagined. Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/reimagined) para obtener más detalles.
:::
...
Para usar Ollama Cloud con Reimagined:
...
3. Copie la clave API para usarla en Reimagined.
...
6. **Importante**: Antes de usar modelos en la nube en Reimagined, debe obtener la información del modelo localmente:
...
Reimagined Zen es una lista de modelos probados y verificados proporcionada por el equipo Reimagined. [Más información](/docs/zen).

1. Inicie sesión en **<a href={console}>Reimagined Zen</a>** y haga clic en **Crear API clave**.

2. Ejecute el comando `/connect` y busque **Reimagined Zen**.

   ```txt
   /connect
   ```

3. Ingrese su clave Reimagined API.
...
4. Muchos modelos de OpenRouter están precargados de forma predeterminada, ejecute el comando `/models` para seleccionar el que desee.
   ...
   También puede agregar modelos adicionales a través de su configuración reimagined.
...
5. También puedes personalizarlos a través de tu configuración reimagined. A continuación se muestra un ejemplo de cómo especificar un proveedor.
...
Para utilizar [API generativas de Scaleway](https://www.scaleway.com/en/docs/generative-apis/) con Reimagined:
...
También puede personalizar modelos a través de su configuración reimagined. A continuación se muestra un ejemplo de cómo especificar el orden de enrutamiento del proveedor.
...
:::tip
Puede utilizar cualquier proveedor compatible con OpenAI con reimagined. La mayoría de los proveedores de IA modernos ofrecen API compatibles con OpenAI.
:::
...
4. Cree o actualice su archivo `reimagined.json` en el directorio de su proyecto:
...
Los campos `limit` le permiten a Reimagined comprender cuánto contexto le queda. Los proveedores estándar los extraen de models.dev automáticamente.
...
1. **Verifique la configuración de autenticación**: Ejecute `reimagined auth list` para ver si las credenciales
...
2. Para proveedores personalizados, verifique la configuración reimagined y:
   - Asegúrese de que el ID del proveedor utilizado en el comando `/connect` coincida con el ID en su configuración reimagined.
