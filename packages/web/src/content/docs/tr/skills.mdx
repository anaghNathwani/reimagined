...
Ajan becerileri, reimagined'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
reimagined su konumlari tarar:

- Proje konfigurasyonu: `.reimagined/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/reimagined/skills/<name>/SKILL.md`
...
Proje yerel yollarinda reimagined, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.reimagined/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/reimagined/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.reimagined/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
reimagined, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`reimagined.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`reimagined.json` icinde):
