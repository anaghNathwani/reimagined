...
import type { SessionNotFoundError } from "@reimagined-ai/sdk/v2/client"
...
describe("parseReadableConfigInvalidError", () => {
  test("formats issues with file path", () => {
    ...
    expect(result).toBe(
      ["Arquivo de config em reimagined.config.ts invalido: settings.host: Required", "mode: Invalid"].join("\n"),
    )
  })
