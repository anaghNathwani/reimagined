---
title: Fournisseurs
description: Utiliser n’importe quel fournisseur LLM en Reimagined.
---

import config from "../../../../config.mjs"
export const console = config.console

Reimagined utilise [AI SDK](https://ai-sdk.dev/) et [Models.dev](https://models.dev) pour prendre en charge **75+ fournisseurs LLM** et prend en charge l'exécution de modèles locaux.

Pour ajouter un fournisseur, vous devez :

1. Ajoutez les clés API pour le fournisseur à l'aide de la commande `/connect`.
2. Configurez le fournisseur dans votre configuration Reimagined.
...
en `~/.local/share/reimagined/auth.json`.

---

### Configuration

Vous pouvez personnaliser les fournisseurs via la section `provider` de votre Reimagined
configuration.
...
Reimagined Zen est une liste de modèles fournis par l'équipe Reimagined qui ont été
testé et vérifié pour fonctionner correctement avec Reimagined. [En savoir plus](/docs/zen).

:::tip
Si vous êtes nouveau, nous vous recommandons de commencer par Reimagined Zen.
:::

1. Exécutez la commande `/connect` dans le TUI, sélectionnez reimagined et dirigez-vous vers [reimagined.ai/auth](https://reimagined.ai/auth).
...
Il fonctionne comme n’importe quel autre fournisseur dans Reimagined et son utilisation est totalement facultative.

---

## Reimagined Go

Reimagined Go est un plan d'abonnement à faible coût qui offre un accès fiable aux modèles de codage ouverts populaires fournis par l'équipe Reimagined qui ont été
testé et vérifié pour fonctionner correctement avec Reimagined.

1. Exécutez la commande `/connect` dans le TUI, sélectionnez `Reimagined Go` et rendez-vous sur [reimagined.ai/auth](https://reimagined.ai/zen).
...
Pour utiliser Amazon Bedrock avec Reimagined :
...
Définissez l'une de ces variables d'environnement lors de l'exécution de reimagined :
...
Pour une configuration spécifique au projet ou persistante, utilisez `reimagined.json` :
...
:::info
L'utilisation de votre abonnement Claude Pro/Max dans Reimagined n'est pas officiellement prise en charge par [Anthropic](https://anthropic.com).
:::
...
Vous pouvez configurer reimagined pour utiliser des modèles locaux via [Atomic Chat](https://atomic.chat), une application de bureau qui exécute des LLM locaux derrière un serveur API compatible OpenAI (point de terminaison par défaut `http://127.0.0.1:1337/v1`).
...
2. Accédez à [Azure AI Foundry](https://ai.azure.com/) et déployez un modèle.
   ...
   :::note
   Le nom du déploiement doit correspondre au nom du modèle pour que reimagined fonctionne correctement.
   :::
...
Vous pouvez également ajouter des modèles via votre configuration reimagined.
...
pour stocker le jeton dans le stockage d'authentification reimagined.
:::

##### GitLab auto-hébergé

:::note[note de conformité]
Reimagined utilise un petit modèle pour certaines tâches d'IA telles que la génération du titre de la session.
Il est configuré pour utiliser gpt-5-nano par défaut, hébergé par Zen. Pour verrouiller Reimagined
pour utiliser uniquement votre propre instance hébergée par GitLab, ajoutez ce qui suit à votre
Fichier `reimagined.json`. Il est également recommandé de désactiver le partage de session.
...
Plus de documentation sur la page d'accueil [reimagined-gitlab-auth](https://www.npmjs.com/package/reimagined-gitlab-auth).

##### Configuration

Personnalisez via `reimagined.json` :
...
Pour utiliser votre abonnement GitHub Copilot avec reimagined :
...
Pour utiliser Google Vertex AI avec Reimagined :
...
Définissez-les lors de l'exécution de reimagined.
...
Si vous voyez une fonctionnalité ou un modèle d'Helicone qui n'est pas configuré automatiquement via reimagined, vous pouvez toujours le configurer vous-même.
...
La fonctionnalité [Sessions](https://docs.helicone.ai/features/sessions) d'Helicone vous permet de regrouper les requêtes LLM associées. Utilisez le plugin [reimagined-helicone-session](https://github.com/H2Shami/reimagined-helicone-session) pour enregistrer automatiquement chaque conversation Reimagined en tant que session dans Helicone.
...
Le plugin injecte les en-têtes `Helicone-Session-Id` et `Helicone-Session-Name` dans vos requêtes. Sur la page Sessions d'Helicone, vous verrez chaque conversation Reimagined répertoriée comme une session distincte.
...
Vous pouvez configurer reimagined pour utiliser des modèles locaux via l'utilitaire llama-server de [llama.cpp's](https://github.com/ggml-org/llama.cpp)
...
Vous pouvez configurer reimagined pour utiliser des modèles locaux via LM Studio.
...
Vous pouvez configurer reimagined pour utiliser des modèles locaux via Ollama.

:::tip
Ollama peut se configurer automatiquement pour Reimagined. Voir les [documents d'intégration Ollama](https://docs.ollama.com/integrations/reimagined) pour plus de détails.
:::
...
Pour utiliser Ollama Cloud avec Reimagined :
...
3. Copiez la clé API à utiliser dans Reimagined.
...
6. **Important** : Avant d'utiliser des modèles cloud dans Reimagined, vous devez extraire les informations du modèle localement :
...
Reimagined Zen est une liste de modèles testés et vérifiés fournie par l'équipe Reimagined. [En savoir plus](/docs/zen).

1. Connectez-vous à **<a href={console}>Reimagined Zen</a>** et cliquez sur **Créer une clé API**.

2. Exécutez la commande `/connect` et recherchez **Reimagined Zen**.

   ```txt
   /connect
   ```

3. Entrez votre clé Reimagined API.
...
Vous pouvez également ajouter des modèles supplémentaires via votre configuration reimagined.
...
5. Vous pouvez également les personnaliser via votre configuration reimagined. Voici un exemple de spécification d'un fournisseur
...
Vous pouvez également personnaliser les modèles via votre configuration reimagined. Voici un exemple de spécification de l'ordre de routage du fournisseur.
...
:::tip
Vous pouvez utiliser n'importe quel fournisseur compatible OpenAI avec reimagined. La plupart des fournisseurs d'IA modernes proposent des API compatibles OpenAI.
:::
...
4. Créez ou mettez à jour votre fichier `reimagined.json` dans le répertoire de votre projet :
...
Les champs `limit` permettent à Reimagined de comprendre la quantité de contexte qu'il vous reste. Les fournisseurs standard les extraient automatiquement de models.dev.
...
1. **Vérifiez la configuration de l'authentification** : exécutez `reimagined auth list` pour voir si les informations d'identification
...
2. Pour les fournisseurs personnalisés, vérifiez la configuration reimagined et :
   - Assurez-vous que l'ID du fournisseur utilisé dans la commande `/connect` correspond à l'ID de votre configuration reimagined.
