...
Practical reference for a future `packages/server` split after the reimagined
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/reimagined`.
...
Until enough shared service code lives outside `packages/reimagined`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/reimagined`

It should not import `packages/reimagined` services while `packages/reimagined`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on reimagined-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/reimagined` implementation details.
