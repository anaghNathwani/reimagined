---
title: Dodaci
description: Napišite vlastite dodatke za proširenje Reimagined.
---

Dodaci vam omogućavaju da proširite Reimagined spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja Reimagined.
...
- `.reimagined/plugins/` - Dodaci na nivou projekta
- `~/.config/reimagined/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/reimagined/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/reimagined/reimagined.json`)
2. Konfiguracija projekta (`reimagined.json`)
3. Globalni direktorij dodataka (`~/.config/reimagined/plugins/`)
4. Direktorij dodataka projekta (`.reimagined/plugins/`)
...
Reimagined pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje Reimagined.
...
Spriječite reimagined da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u reimagined:
...
Pomoćnik `tool` kreira prilagođeni alat koji reimagined može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za reimagined zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://reimagined.ai/docs/sdk) za detalje.
