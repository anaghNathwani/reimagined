---
title: Complementos
description: Escriba sus propios complementos para extender Reimagined.
---

Los complementos le permiten extender Reimagined al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de Reimagined.
...
- `.reimagined/plugins/` - Complementos a nivel de proyecto
- `~/.config/reimagined/plugins/` - Complementos globales
...
```json title="reimagined.json"
{
  ...
  "plugin": ["reimagined-helicone-session", "reimagined-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/reimagined/node_modules/`.
...
1. Configuración global (`~/.config/reimagined/reimagined.json`)
2. Configuración del proyecto (`reimagined.json`)
3. Directorio global de complementos (`~/.config/reimagined/plugins/`)
4. Directorio de complementos del proyecto (`.reimagined/plugins/`)
...
```json title=".reimagined/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
Reimagined ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".reimagined/plugins/my-plugin.ts"
...
```js title=".reimagined/plugins/example.js"
...
- `client`: Un cliente SDK reimagined para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar reimagined.
...
```js title=".reimagined/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "reimagined"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio Reimagined, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que reimagined lea archivos `.env`:

```javascript title=".reimagined/plugins/env-protection.js"
...
```javascript title=".reimagined/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a reimagined:

```ts title=".reimagined/plugins/custom-tools.ts"
import { type Plugin, tool } from "@reimagined-ai/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que reimagined puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para reimagined junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://reimagined.ai/docs/sdk) para obtener más detalles.
...
```ts title=".reimagined/plugins/compaction.ts"
import type { Plugin } from "@reimagined-ai/plugin"
...
```ts title=".reimagined/plugins/custom-compaction.ts"
import type { Plugin } from "@reimagined-ai/plugin"
