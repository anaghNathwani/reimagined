...
Du kan legge til eksterne verktøy i Reimagined ved å bruke _Model Context Protocol_ eller MCP. Reimagined støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [Reimagined Config](https://reimagined.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="reimagined.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/reimagined`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
```jsonc title="reimagined.jsonc" {15}
{
  "$schema": "https://reimagined.ai/config.json",
  "mcp": {
    "my-local-mcp-server": {
...
Reimagined håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil Reimagined:
...
Hvis serveren krever autentisering, vil Reimagined be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `reimagined mcp auth <server-name>`.
...
```json title="reimagined.json" {7-11}
{
  "$schema": "https://reimagined.ai/config.json",
  "mcp": {
    "my-oauth-server": {
...
```bash
reimagined mcp auth my-oauth-server
```
...
```bash
reimagined mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil Reimagined lagre tokens sikkert i `~/.local/share/reimagined/mcp-auth.json`.
...
reimagined mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i Reimagined, sammen med innebygde verktøy. Så du kan administrere dem gjennom Reimagined-konfigurasjonen som alle andre verktøy.
...
```json title="reimagined.json" {11, 14-18}
{
  "$schema": "https://reimagined.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```json title="reimagined.json" {4-8}
{
  "$schema": "https://reimagined.ai/config.json",
  "mcp": {
    "sentry": {
...
```bash
reimagined mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble Reimagined til Sentry-kontoen din.
...
```json title="reimagined.json" {4-7}
{
  "$schema": "https://reimagined.ai/config.json",
  "mcp": {
    "context7": {
...
```json title="reimagined.json" {7-9}
{
  "$schema": "https://reimagined.ai/config.json",
  "mcp": {
    "context7": {
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver Reimagined-samtale oppført som en separat økt.
