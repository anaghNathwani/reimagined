...
Med Reimagined kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger Reimagined vårt eget `reimagined`-tema.
...
Reimagined støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/reimagined/themes/*.json` eller `$XDG_CONFIG_HOME/reimagined/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.reimagined/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.reimagined/themes/*.json`
