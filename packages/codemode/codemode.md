...
This is the living design and status document for `@reimagined-ai/codemode` and its existing V2 Reimagined adapter.
...
### V2 Reimagined adapter

This section describes the `v2` branch integration. On `dev`, CodeMode is integrated through
`packages/reimagined/src/tool/code-mode.ts`, where nested MCP calls run the `tool.execute.before` and
