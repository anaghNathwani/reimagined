---
title: Server
description: Interager med reimagined-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `reimagined serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en reimagined-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `reimagined.local` |
...
Indstil `OPENCODE_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `reimagined`, eller indstil `OPENCODE_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `reimagined serve` og `reimagined web`.
...
Når du kører `reimagined` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug reimagined-serveren til at interagere med reimagined programmatisk.
:::

Denne arkitektur lader reimagined understøtte flere klienter og lader dig interagere med reimagined programmatisk.

Du kan køre `reimagined serve` for at starte en selvstændig server. Hvis du har
reimagined TUI kørende, vil `reimagined serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af Reimagined [IDE](/docs/ide) plugins.
...
Reimagined-serveren viser følgende API'er.
