...
Avec Reimagined, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, Reimagined utilise notre propre thème `reimagined`.
...
Reimagined est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que Reimagined corresponde à l'apparence de leur terminal
...
Reimagined prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/reimagined/themes/*.json` ou `$XDG_CONFIG_HOME/reimagined/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.reimagined/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.reimagined/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/reimagined/themes
vim ~/.config/reimagined/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .reimagined/themes
vim .reimagined/themes/my-theme.json
```
...
{
  "$schema": "https://reimagined.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
