---
title: Sağlayıcılar
description: reimagined'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

reimagined, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı reimagined yapılandırmanızda yapılandırın.
...
`~/.local/share/reimagined/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları reimagined'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
Reimagined Zen, reimagined ekibi tarafından sağlanan modellerin bir listesidir.
reimagined ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz Reimagined Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, reimagined'u seçin ve [reimagined.ai/auth](https://reimagined.ai/auth)'ye gidin.
...
reimagined'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## Reimagined Go

Reimagined Go, reimagined ile iyi çalıştığı test edilmiş ve doğrulanmış, reimagined ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `Reimagined Go`'yu seçin ve [reimagined.ai/auth](https://reimagined.ai/zen) adresine gidin.
...
Amazon Bedrock'u reimagined ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   reimagined'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `reimagined.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi reimagined'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
reimagined'u, yerel LLM'leri OpenAI uyumlu bir API sunucusunun arkasında çalıştıran bir masaüstü uygulaması olan [Atomic Chat](https://atomic.chat) aracılığıyla yerel modelleri kullanacak şekilde yapılandırabilirsiniz (varsayılan uç nokta `http://127.0.0.1:1337/v1`).
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   reimagined'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
:::note[uyumluluk notu]
reimagined, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. reimagined'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`reimagined.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
Daha fazla belge [reimagined-gitlab-auth](https://www.npmjs.com/package/reimagined-gitlab-auth) ana sayfasında.

##### Yapılandırma

`reimagined.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi reimagined'la kullanmak için:
...
Google Vertex AI'yi reimagined ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları reimagined çalıştırılırken ayarlayın.
...
Helicone'un reimagined aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her reimagined görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [reimagined-helicone-session](https://github.com/H2Shami/reimagined-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her reimagined görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
reimagined'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla reimagined'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla reimagined'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini reimagined için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/reimagined)'a bakın.
:::
...
Ollama Cloud'u reimagined ile kullanmak için:
...
3. reimagined'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: reimagined'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
Reimagined Zen, reimagined ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>Reimagined Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **Reimagined Zen**'i arayın.

   ```txt
   /connect
   ```

3. reimagined API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   reimagined yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca reimagined yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
Modelleri reimagined yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı reimagined ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `reimagined.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları reimagined'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `reimagined auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için reimagined yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, reimagined yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
