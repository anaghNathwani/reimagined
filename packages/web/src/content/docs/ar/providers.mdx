---
title: المزوّدون
description: استخدام أي موفّر LLM في Reimagined.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم Reimagined ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات Reimagined.
...
في `~/.local/share/reimagined/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات Reimagined.
...
Reimagined Zen هي قائمة نماذج يوفّرها فريق Reimagined وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع Reimagined. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع Reimagined Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر reimagined، ثم انتقل إلى [reimagined.ai/auth](https://reimagined.ai/auth).
...
يعمل مثل أي مزوّد آخر في Reimagined واستخدامه اختياري بالكامل.

---

## Reimagined Go

Reimagined Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق Reimagined، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع Reimagined.

1. شغّل الأمر `/connect` في TUI، واختر `Reimagined Go`، ثم انتقل إلى [reimagined.ai/auth](https://reimagined.ai/zen).
...
لاستخدام Amazon Bedrock مع Reimagined:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل reimagined:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `reimagined.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل Reimagined غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين reimagined لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل reimagined بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات reimagined.
...
تخزين الرمز في مساحة تخزين مصادقة reimagined.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم Reimagined نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر Reimagined
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`reimagined.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [reimagined-gitlab-auth](https://www.npmjs.com/package/reimagined-gitlab-auth).

##### التهيئة

خصّص عبر `reimagined.json`:
...
لاستخدام اشتراك GitHub Copilot مع reimagined:
...
لاستخدام Google Vertex AI مع Reimagined:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر reimagined، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [reimagined-helicone-session](https://github.com/H2Shami/reimagined-helicone-session) لتسجيل كل محادثة Reimagined تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة Reimagined مدرجة كجلسة مستقلة.
...
يمكنك ضبط reimagined لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط reimagined لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط reimagined لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع Reimagined. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/reimagined) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع Reimagined:
...
3. انسخ مفتاح API لاستخدامه في Reimagined.
...
6. **مهم**: قبل استخدام نماذج السحابة في Reimagined، يجب سحب معلومات النموذج محليا:
...
Reimagined Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق Reimagined. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>Reimagined Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **Reimagined Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ Reimagined.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات reimagined.
...
5. يمكنك أيضا تخصيصها عبر إعدادات reimagined. إليك مثالا على تحديد مزوّد
...
يمكنك أيضا تخصيص النماذج عبر إعدادات reimagined. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع reimagined. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `reimagined.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ Reimagined بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `reimagined auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات reimagined وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات reimagined.
