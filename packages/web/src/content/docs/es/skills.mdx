...
Las habilidades del agente permiten a Reimagined descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
Reimagined busca estas ubicaciones:

- Configuración del proyecto: `.reimagined/skills/<name>/SKILL.md`
- Configuración global: `~/.config/reimagined/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, Reimagined sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.reimagined/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/reimagined/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.reimagined/skills/git-release/SKILL.md` así:
...
Reimagined enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `reimagined.json`:
...
**Para agentes integrados** (en `reimagined.json`):
