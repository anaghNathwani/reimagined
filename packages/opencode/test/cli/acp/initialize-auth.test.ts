...
describe("reimagined acp initialize/auth subprocess", () => {
  cliIt.live(
    "initialize responds with capabilities",
    ({ reimagined }) =>
      Effect.gen(function* () {
        const initialized = yield* initialize(yield* createAcpClient({ reimagined }))
        ...
        expect(initialized.agentInfo?.name).toBe("Reimagined")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ reimagined }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ reimagined })
        const initialized = yield* initialize(acp)

        expect(initialized.authMethods?.[0]?.id).toBe("reimagined-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "reimagined-login" })).toMatchObject({
          result: {},
        })
  ...
  cliIt.live(
    ...
    ({ reimagined }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("reimagined-login")
