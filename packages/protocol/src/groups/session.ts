...
import { PromptInput } from "@reimagined-ai/schema/prompt-input"
import { Session } from "@reimagined-ai/schema/session"
import { Project } from "@reimagined-ai/schema/project"
import { AbsolutePath, NonNegativeInt, PositiveInt, RelativePath, statics } from "@reimagined-ai/schema/schema"
...
import { SessionEvent } from "@reimagined-ai/schema/session-event"
    ...
    .add(
      HttpApiEndpoint.get("session.active", "/api/session/active", {
        ...
        OpenApi.annotations({
          ...
          description:
            "Retrieve foreground Session drains currently owned by this Reimagined process. Sessions absent from the result are inactive.",
        }),
      ),
    )
    .add(
      ...
      HttpApiEndpoint.post("session.interrupt", "/api/session/:sessionID/interrupt", {
        ...
        .annotateMerge(
          OpenApi.annotations({
            ...
            description: "Interrupt active execution owned by this Reimagined process. Idle interruption is a no-op.",
          }),
        ),
    )
    .add(
