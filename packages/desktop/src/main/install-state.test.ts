...
describe("hasExistingAppState", () => {
  ...
  test("recognizes state written by an earlier Reimagined launch", () => {
    expect(hasExistingAppState([file("reimagined.settings")])).toBe(true)
    expect(hasExistingAppState([file("reimagined.global.dat")])).toBe(true)
    expect(hasExistingAppState([file("window-state-abc.json")])).toBe(true)
    expect(hasExistingAppState([directory("reimagined")])).toBe(true)
  })
})
