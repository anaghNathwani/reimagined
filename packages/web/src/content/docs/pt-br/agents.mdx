...
Existem dois tipos de agentes no reimagined; agentes primários e subagentes.
...
reimagined vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
Reimagined vem com três subagentes integrados, **General**, **Explore** e **Scout**. Vamos ver isso abaixo.

---

## Integrados

Reimagined vem com dois agentes primários integrados e três subagentes integrados.
...
Um agente somente leitura para pesquisa em documentação externa e dependências. Use-o quando você precisar clonar o repositório de uma dependência para o cache gerenciado do Reimagined, inspecionar o código-fonte de uma biblioteca ou cruzar o código local com implementações upstream sem modificar seu workspace.
...
Configure os agentes em seu arquivo de configuração `reimagined.json`:
...
- Global: `~/.config/reimagined/agents/`
- Por projeto: `.reimagined/agents/`

```markdown title="~/.config/reimagined/agents/review.md"
---
...
Se nenhuma temperatura for especificada, o reimagined usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do reimagined quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do reimagined usa o formato `provider/model-id`. Por exemplo, se você estiver usando [Reimagined Zen](/docs/zen), você usaria `reimagined/gpt-5.1-codex` para GPT 5.1 Codex.
...
```json title="reimagined.json" {3-6,9-12}
{
  "$schema": "https://reimagined.ai/config.json",
  "tools": {
    "write": true,
...
```json title="reimagined.json" {3-5,8-10}
{
  "$schema": "https://reimagined.ai/config.json",
  "permission": {
    "edit": "deny"
...
```json title="reimagined.json" {6,7}
{
  "agent": {
    "deep-thinker": {
...
:::tip
Execute `reimagined models` para ver uma lista dos modelos disponíveis.
:::
...
:::tip
Você tem um agente que gostaria de compartilhar? [Envie um PR](https://github.com/anomalyco/reimagined).
:::
...
```markdown title="~/.config/reimagined/agents/docs-writer.md"
---
...
```markdown title="~/.config/reimagined/agents/security-auditor.md"
---
