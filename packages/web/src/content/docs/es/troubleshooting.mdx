...
Para depurar problemas con Reimagined, comience verificando los registros y los datos locales que almacena en el disco.
...
- **macOS/Linux**: `~/.local/share/reimagined/log/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\reimagined\log`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `reimagined --log-level DEBUG`.

---

## Almacenamiento

reimagined almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/reimagined/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\reimagined`
...
Reimagined Desktop ejecuta un servidor Reimagined local (el sidecar `reimagined-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `Reimagined` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **macOS/Linux**: `~/.config/reimagined/reimagined.jsonc` (o `~/.config/reimagined/reimagined.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/reimagined/reimagined.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\reimagined\reimagined.jsonc`
...
Reimagined también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:

- **Complementos globales**
  - **macOS/Linux**: `~/.config/reimagined/plugins/`
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\reimagined\plugins`
- **Complementos de proyecto** (solo si usas la configuración por proyecto)
  - `<your-project>/.reimagined/plugins/`
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que Reimagined pueda reconstruirlo.

1. Salga de Reimagined Desktop por completo.
2. Elimine el directorio de caché:

- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/reimagined`
- **Linux**: eliminar `~/.cache/reimagined` (o ejecutar `rm -rf ~/.cache/reimagined`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\reimagined`

3. Reinicie el escritorio Reimagined.

---

### Solucionar problemas de conexión del servidor

Reimagined Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si su `reimagined.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.
...
En Windows, el escritorio Reimagined requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de Reimagined.

---

### Notificaciones que no se muestran

Reimagined Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para Reimagined en la configuración de su sistema operativo, y
...
1. Salga del escritorio Reimagined.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio Reimagined):

- `reimagined.settings.dat` (URL del servidor predeterminado de escritorio)
- `reimagined.global.dat` y `reimagined.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
Si tiene problemas con Reimagined:
...
1. **Informar problemas el GitHub**
   ...
   [**github.com/anomalyco/reimagined/issues**](https://github.com/anomalyco/reimagined/issues)
...
2. **Únete a nuestro Discord**
   ...
   [**reimagined.ai/discord**](https://reimagined.ai/discord)
...
3. Asegúrese de tener la última versión con `reimagined upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `reimagined models`
...
2. Si el problema persiste, intente borrar la configuración almacenada:
   ...
   ```bash
   rm -rf ~/.local/share/reimagined
   ```

   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.local\share\reimagined`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. reimagined instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
1. Borre la caché del paquete del proveedor:
   ...
   ```bash
   rm -rf ~/.cache/reimagined
   ```

   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.cache\reimagined`

2. Reinicie reimagined para reinstalar los paquetes del proveedor más recientes.

Esto obligará a reimagined a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
reimagined detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
