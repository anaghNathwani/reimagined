---
title: Server
description: Komunicirajte s reimagined serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `reimagined serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji reimagined klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `reimagined.local` |
...
Postavite `OPENCODE_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `reimagined`, ili postavite `OPENCODE_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `reimagined serve` i za `reimagined web`.
...
Kada pokrenete `reimagined`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite reimagined server za programsku interakciju sa reimagined-om.
:::

Ova arhitektura omogucava reimagined podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `reimagined serve` da startate standalone server. Ako je reimagined TUI vec pokrenut, `reimagined serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste Reimagined [IDE](/docs/ide) pluginovi.
...
reimagined server izlaže sljedece API-je.
