...
Możesz dodać dodatkowe informacje do reimagined za pomocą _Model Context Protocol_, w skrócie MCP. reimagined obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [reimagined Config](https://reimagined.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/reimagined`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
reimagined automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, reimagined:
...
Jeśli serwer wymaga uwierzytelnienia, reimagined poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `reimagined mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji reimagined bezpieczne przechowa tokeny w `~/.local/share/reimagined/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w reimagined, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie reimagined, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć reimagined z kontem Sentry.
