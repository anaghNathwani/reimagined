---
title: Providers
description: Using any LLM provider in Reimagined.
---

import config from "../../../config.mjs"
export const console = config.console

Reimagined uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your Reimagined config.
...
in `~/.local/share/reimagined/auth.json`.

---

### Config

You can customize the providers through the `provider` section in your Reimagined
config.
...
Reimagined Zen is a list of models provided by the Reimagined team that have been
tested and verified to work well with Reimagined. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with Reimagined Zen.
:::

1. Run the `/connect` command in the TUI, select `Reimagined Zen`, and head to [reimagined.ai/auth](https://reimagined.ai/zen).
...
It works like any other provider in Reimagined and is completely optional to use.

---

## Reimagined Go

Reimagined Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the Reimagined team that have been
tested and verified to work well with Reimagined.

1. Run the `/connect` command in the TUI, select `Reimagined Go`, and head to [reimagined.ai/auth](https://reimagined.ai/zen).
...
To use Amazon Bedrock with Reimagined:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   Set one of these environment variables while running reimagined:

   ```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY reimagined

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile reimagined

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX reimagined
   ```
   ...
   ***
   ...
   For project-specific or persistent configuration, use `reimagined.json`:

   ```json title="reimagined.json"
   {
     "$schema": "https://reimagined.ai/config.json",
     "provider": {
       "amazon-bedrock": {
...
Reimagined. Anthropic explicitly prohibits this.

Previous versions of Reimagined came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in Reimagined with zero setup:
...
You can configure reimagined to use local models through [Atomic Chat](https://atomic.chat), a desktop application that runs local LLMs behind an OpenAI-compatible API server (default endpoint `http://127.0.0.1:1337/v1`).

```json title="reimagined.json" "atomic-chat" {5, 6, 8, 10-14}
{
  "$schema": "https://reimagined.ai/config.json",
  "provider": {
    "atomic-chat": {
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for reimagined to work properly.
   :::
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX reimagined
   ```
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX reimagined
   ```
...
6. Run the `/models` command to select a model.
   ...
   You can also add models through your reimagined config.
...
Reimagined supports two authentication methods:

- **OAuth (Recommended)** — Sign in to your DigitalOcean account; Reimagined uses your DigitalOcean API token directly for inference and discovers your Inference Routers.
...
3. Your browser opens to authorize Reimagined. Sign in and approve.

   :::note
   Reimagined requests `genai:read` and `inference:query` OAuth scopes. Your DigitalOcean API token is used directly for inference — no separate Model Access Key is created.
   :::
...
Inference Routers let you define a routing policy across multiple models — picking the cheapest, fastest, or most appropriate model per request based on the task. After OAuth, Reimagined surfaces each router as `router:<router-name>` in the model picker.

Selecting a router model is a drop-in replacement for any other model — Reimagined forwards your request and DigitalOcean picks the underlying model based on your router's policy. Learn more about [Inference Routers](https://docs.digitalocean.com/products/inference/how-to/use-inference-router/)
...
GitLab Duo support in Reimagined is experimental. Features, configuration, and
behavior may change in future releases.
:::

Reimagined integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
...
to store token in reimagined auth storage.
:::

##### Self-Hosted GitLab

:::note[compliance note]
Reimagined uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock Reimagined
to only use your own GitLab-hosted instance, add the following to your
`reimagined.json` file. It is also recommended to disable session sharing.
...
More documentation on [reimagined-gitlab-auth](https://www.npmjs.com/package/reimagined-gitlab-auth) homepage.

##### Configuration

Customize through `reimagined.json`:
...
When a `duo-workflow-*` model is selected, Reimagined will:
...
4. Route tool execution requests through Reimagined's permission-gated tool system
...
{
  "$schema": "https://reimagined.ai/config.json",
  "plugin": ["reimagined-gitlab-plugin"]
}
```
...
To use your GitHub Copilot subscription with reimagined:
...
To use GMI Cloud with Reimagined:
...
To use Google Vertex AI with Reimagined:
...
2. Set the required environment variables:
   ...
   Set them while running reimagined.

   ```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id reimagined
   ```
...
In the event you see a feature or model from Helicone that isn't configured automatically through reimagined, you can always configure it yourself.
...
```jsonc title="~/.config/reimagined/reimagined.jsonc"
{
  "$schema": "https://reimagined.ai/config.json",
  "provider": {
    "helicone": {
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [reimagined-helicone-session](https://github.com/H2Shami/reimagined-helicone-session) plugin to automatically log each Reimagined conversation as a session in Helicone.

```bash
npm install -g reimagined-helicone-session
```
...
```json title="reimagined.json"
{
  "plugin": ["reimagined-helicone-session"]
}
```

The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each Reimagined conversation listed as a separate session.
...
You can configure reimagined to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility

```json title="reimagined.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://reimagined.ai/config.json",
  "provider": {
    "llama.cpp": {
...
You can configure reimagined to use local models through LM Studio.

```json title="reimagined.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://reimagined.ai/config.json",
  "provider": {
    "lmstudio": {
...
You can configure reimagined to use local models through Ollama.

:::tip
Ollama can automatically configure itself for Reimagined. See the [Ollama integration docs](https://docs.ollama.com/integrations/reimagined) for details.
:::

```json title="reimagined.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://reimagined.ai/config.json",
  "provider": {
    "ollama": {
...
To use Ollama Cloud with Reimagined:
...
3. Copy the API key for use in Reimagined.
...
6. **Important**: Before using cloud models in Reimagined, you must pull the model information locally:
...
Reimagined Zen is a list of tested and verified models provided by the Reimagined team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>Reimagined Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **Reimagined Zen**.

   ```txt
   /connect
   ```

3. Enter your Reimagined API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your reimagined config.
...
5. You can also customize them through your reimagined config. Here's an example of specifying a provider
...
3. Enter your service key JSON.
   ...
   ```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' reimagined
   ```
...
4. Optionally set deployment ID and resource group:
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group reimagined
   ```
...
Don't have a Snowflake account? [Sign up for a free trial](https://signup.snowflake.com/?utm_source=reimagined&utm_medium=docs&utm_campaign=cortex-provider).

reimagined's core workflow for coding, editing files, and running commands relies on tool calling. Only the Claude and OpenAI families within Snowflake Cortex support this. The provider is limited to those families to support the core workflow.
...
5. Complete sign-in in the browser that opens. Reimagined captures the OAuth callback automatically and stores the token — no copy/paste needed.
...
For CI or headless environments, set a PAT or JWT before starting reimagined:
...
The model catalog is provided automatically. A minimal `reimagined.json` is all that's needed:
...
You can also customize models through your reimagined config. Here's an example of specifying provider routing order.
...
2. Select **xAI Grok OAuth (SuperGrok Subscription)**. Reimagined opens xAI's consent screen in your browser and waits for the callback on `http://127.0.0.1:56121/callback`.
...
Reimagined refreshes the OAuth access token automatically. Any Grok or X Premium plan that includes Grok API access works; you do not need a separate `XAI_API_KEY`.

#### Option B — SuperGrok device-code (headless / remote server / VPS)

Use this when Reimagined is running somewhere a browser can't reach the loopback redirect: a VPS, a remote dev box over SSH, inside Docker, in CI, etc. No callback port is opened on the host running Reimagined — instead xAI hands the CLI a short code that you type into a browser on any other device (laptop, phone, …).
...
2. Select **xAI Grok OAuth (Headless / Remote / VPS)**. Reimagined prints a verification URL and a short user code.

   ```txt
   Open https://x.ai/device on any device and enter code: ABCD-1234
   ```

3. Open the URL on a device that has a browser (your laptop or phone), enter the code, and approve the consent screen. Reimagined polls xAI's token endpoint and stores the resulting OAuth tokens once you approve. Token refresh works the same as Option A.
...
:::tip
You can use any OpenAI-compatible provider with reimagined. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
3. Enter your API key for the provider.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in reimagined.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Create or update your `reimagined.json` file in your project directory:

   ```json title="reimagined.json" ""myprovider"" {5-15}
   {
     "$schema": "https://reimagined.ai/config.json",
     "provider": {
       "myprovider": {
...
```json title="reimagined.json" {9,11,17-20}
{
  "$schema": "https://reimagined.ai/config.json",
  "provider": {
    "myprovider": {
...
The `limit` fields allow Reimagined to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `reimagined auth list` to see if the credentials
...
2. For custom providers, check the reimagined config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your reimagined config.
