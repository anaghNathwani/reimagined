---
title: Serveur
description: Interagissez avec le serveur reimagined via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `reimagined serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client reimagined peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `reimagined.local` |
...
Définissez `OPENCODE_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `reimagined`, ou définissez `OPENCODE_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `reimagined serve` et à `reimagined web`.
...
Lorsque vous exécutez `reimagined`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur reimagined pour interagir avec reimagined par programmation.
:::

Cette architecture permet à reimagined de prendre en charge plusieurs clients et vous permet d'interagir avec reimagined par programme.

Vous pouvez exécuter `reimagined serve` pour démarrer un serveur autonome. Si vous avez le
reimagined TUI en cours d'exécution, `reimagined serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins Reimagined [IDE](/docs/ide).
...
Le serveur reimagined expose les API suivantes.
