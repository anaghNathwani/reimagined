...
Com o reimagined, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o reimagined usa nosso próprio tema `reimagined`.
...
O reimagined vem com vários temas embutidos.
...
- Querem que o reimagined corresponda à aparência do seu terminal
...
O reimagined suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/reimagined/themes/*.json` ou `$XDG_CONFIG_HOME/reimagined/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.reimagined/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.reimagined/themes/*.json`
