---
title: CLI
description: Reimagined CLI muligheder og kommandoer.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Reimagined CLI starter som standard [TUI](/docs/tui), når den kører uden nogen argumenter.

```bash
reimagined
```

Men den accepterer også kommandoer som dokumenteret på denne side. Dette giver dig mulighed for at interagere med Reimagined programmatisk.
...
Start Reimagined terminalbrugergrænsefladen.
...
Reimagined CLI har også følgende kommandoer.

---

### agent

Administrer agenter i Reimagined.
...
Tilslut en terminal til en allerede kørende Reimagined backend-server startet via `serve` eller `web` kommandoer.

```bash
reimagined attach [url]
```

Dette gør det muligt at bruge TUI med en ekstern Reimagined backend. For eksempel:
...
| <nobr><code>{"--username"}</code></nobr> | `-u` | Brugernavn til basic auth (standard: `OPENCODE_SERVER_USERNAME` eller `reimagined`) |
...
Reimagined drives af udbyderlisten på [Models.dev](https://models.dev), så du kan bruge `reimagined auth login` til at konfigurere API-nøgler for enhver udbyder, du vil bruge. Dette er gemt i `~/.local/share/reimagined/auth.json`.

```bash
reimagined auth login
```

Når Reimagined starter op, indlæser den udbyderne fra legitimationsfilen. Og hvis der er nogen nøgler defineret i dine miljøer eller en `.env` fil i dit projekt.
...
Brug flaget `--refresh` til at opdatere den cachelagrede modelliste. Dette er nyttigt, når nye modeller er blevet tilføjet til en udbyder, og du vil se dem i Reimagined.
...
Kør reimagined i ikke-interaktiv tilstand ved at sende en prompt direkte.
...
Du kan også vedhæfte til en kørende `reimagined serve`-instans for at undgå MCP serverens kolde opstartstider ved hver kørsel:
...
| <nobr><code>{"--attach"}</code></nobr>   |      | Tilslut til en kørende reimagined-server (f.eks. http://localhost:4096)               |
| <nobr><code>{"--password"}</code></nobr> | `-p` | Adgangskode til basic auth (standard: `OPENCODE_SERVER_PASSWORD`)                   |
| <nobr><code>{"--username"}</code></nobr> | `-u` | Brugernavn til basic auth (standard: `OPENCODE_SERVER_USERNAME` eller `reimagined`)   |
...
Start en hovedløs Reimagined-server til API-adgang. Tjek [server docs](/docs/server) for den fulde HTTP-grænseflade.

```bash
reimagined serve
```

Dette starter en HTTP-server, der giver API-adgang til reimagined-funktionalitet uden TUI-grænsefladen. Indstil `OPENCODE_SERVER_PASSWORD` for at aktivere HTTP grundlæggende godkendelse (brugernavn er standard til `reimagined`).
...
Administrator Reimagined sessionsholder.
...
Liste over alle Reimagined sessioner.
...
Vis tokenbrug og omkostningsstatistikker for dine Reimagined-sessioner.
...
Importer sessionsdata fra en JSON fil eller Reimagined del URL.

```bash
reimagined import <file>
```

Du kan importere fra en lokal fil eller en Reimagined share URL.
...
Start en hovedløs Reimagined-server med en webgrænseflade.

```bash
reimagined web
```

Dette starter en HTTP-server og åbner en webbrowser for at få adgang til Reimagined via en webgrænseflade. Indstil `OPENCODE_SERVER_PASSWORD` for at aktivere HTTP grundlæggende godkendelse (brugernavn er standard til `reimagined`).
...
Afinstaller Reimagined og fjern alle relaterede filer.
...
Opdaterer Reimagined til den nyeste version eller en specifik version.
...
reimagined CLI tager følgende globale flag.
...
Reimagined kan konfigureres ved hjælp af miljøvariabler.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Tilsidesæt grundlæggende godkendelsesbrugernavn (standard `reimagined`) |
