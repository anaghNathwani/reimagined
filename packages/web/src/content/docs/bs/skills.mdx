...
Agent skills omogucavaju Reimagined da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
Reimagined pretrazuje ove lokacije:

- Konfiguracija projekta: `.reimagined/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/reimagined/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, Reimagined ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.reimagined/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/reimagined/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.reimagined/skills/git-release/SKILL.md` ovako:
...
Reimagined navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `reimagined.json`:
...
**Za ugradene agente** (u `reimagined.json`):
