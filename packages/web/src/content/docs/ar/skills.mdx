...
تتيح مهارات الوكيل لـ Reimagined اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث Reimagined في هذه المواقع:

- إعدادات المشروع: `.reimagined/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/reimagined/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج Reimagined صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.reimagined/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.

كما تُحمَّل التعريفات العامة أيضاً من `~/.config/reimagined/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
أنشئ `.reimagined/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض Reimagined المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `reimagined.json`:
...
**للوكلاء المدمجين** (في `reimagined.json`):
