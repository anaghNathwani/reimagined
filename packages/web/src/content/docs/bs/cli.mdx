---
title: CLI
description: Reimagined CLI opcije i naredbe.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Reimagined CLI po defaultu pokreće [TUI](/docs/tui) kada se pokrene bez ikakvih argumenata.

```bash
reimagined
```

Ali takođe prihvata naredbe kao što je dokumentovano na ovoj stranici. Ovo vam omogućava programsku interakciju sa Reimagined.
...
Pokrenite Reimagined terminalski korisnički interfejs.
...
Reimagined CLI takođe ima sljedeće naredbe.

---

### agent

Upravljajte Reimagined agentima.
...
Priključite terminal na već pokrenut Reimagined backend server pokrenut putem `serve` ili `web` naredbi.

```bash
reimagined attach [url]
```

Ovo omogućava korištenje TUI-ja sa udaljenim Reimagined backend-om. Na primjer:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`   | Korisničko ime za osnovnu autentifikaciju (zadano: `OPENCODE_SERVER_USERNAME` ili `reimagined`) |
...
Reimagined pokreće lista provajdera na [Models.dev](https://models.dev), tako da možete koristiti `reimagined auth login` da konfigurirate API ključeve za bilo kojeg provajdera kojeg želite koristiti. Ovo je pohranjeno u `~/.local/share/reimagined/auth.json`.

```bash
reimagined auth login
```

Kada se Reimagined pokrene, učitava dobavljače iz datoteke vjerodajnica. I ako postoje neki ključevi definirani u vašim okruženjima ili `.env` fajl u vašem projektu.
...
Koristite `--refresh` zastavicu da ažurirate keširanu listu modela. Ovo je korisno kada su novi modeli dodani provajderu i želite da ih vidite u Reimagined.
...
Pokrenite Reimagined u neinteraktivnom modu tako što ćete direktno proslijediti prompt.
...
Također možete priključiti pokrenutu `reimagined serve` instancu kako biste izbjegli vrijeme hladnog pokretanja MCP servera pri svakom pokretanju:
...
| <nobr><code>{"--attach"}</code></nobr>   |        | Priključite na pokrenuti Reimagined server (npr. http://localhost:4096)                         |
| <nobr><code>{"--password"}</code></nobr> | `-p`   | Lozinka za osnovnu autentifikaciju (zadano: `OPENCODE_SERVER_PASSWORD`)                       |
| <nobr><code>{"--username"}</code></nobr> | `-u`   | Korisničko ime za osnovnu autentifikaciju (zadano: `OPENCODE_SERVER_USERNAME` ili `reimagined`) |
...
Pokrenite Reimagined headless server za API pristup. Pogledajte [server docs](/docs/server) za kompletan HTTP interfejs.

```bash
reimagined serve
```

Ovo pokreće HTTP server koji pruža API pristup funkcionalnosti Reimagined-a bez TUI interfejsa. Postavite `OPENCODE_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `reimagined`).
...
Upravljajte Reimagined sesijama.
...
Navedite sve Reimagined sesije.
...
Prikaži statistiku upotrebe tokena i troškova za vaše Reimagined sesije.
...
Uvezite podatke sesije iz JSON datoteke ili Reimagined dijeljenog URL-a.

```bash
reimagined import <file>
```

Možete uvesti iz lokalne datoteke ili Reimagined dijeljenog URL-a.
...
Pokrenite Reimagined headless server sa web interfejsom.

```bash
reimagined web
```

Ovo pokreće HTTP server i otvara web pretraživač za pristup Reimagined-u preko web interfejsa. Postavite `OPENCODE_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `reimagined`).
...
Deinstalirajte Reimagined i uklonite sve povezane datoteke.
...
Ažurira Reimagined na najnoviju verziju ili određenu verziju.
...
Reimagined CLI prihvata sljedeće globalne zastavice.
...
Reimagined se može konfigurirati pomoću varijabli okruženja.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Poništi osnovno korisničko ime autentifikacije (zadano `reimagined`) |
