...
Hay dos tipos de agentes en Reimagined; agentes primarios y subagentes.
...
Reimagined viene con dos agentes principales integrados, **Build** y **Plan**. Bien
...
Reimagined viene con tres subagentes integrados, **General**, **Explore** y **Scout**. Veremos esto a continuación.

---

## Integrados

Reimagined viene con dos agentes primarios integrados y tres subagentes integrados.
...
Configure agentes en su archivo de configuración `reimagined.json`:
...
- Por proyecto: `.reimagined/agents/`
...
Si no se especifica ninguna temperatura, Reimagined utiliza valores predeterminados específicos del modelo; normalmente 0 para la mayoría de los modelos, 0,55 para los modelos Qwen.
...
Esta ruta es relativa a donde se encuentra el archivo de configuración. Entonces esto funciona tanto para la configuración global Reimagined como para la configuración específica del proyecto.
...
El ID del modelo en su configuración Reimagined usa el formato `provider/model-id`. Por ejemplo, si usa [Reimagined Zen](/docs/zen), usaría `reimagined/gpt-5.1-codex` para GPT 5.1 Codex.
...
:::tip
Ejecute `reimagined models` para ver una lista de los modelos disponibles.
:::
...
:::tip
¿Tiene un agente que le gustaría compartir? [Enviar un PR](https://github.com/anomalyco/reimagined).
:::
