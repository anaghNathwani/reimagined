...
As habilidades do agente permitem que o reimagined descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O reimagined pesquisa nesses locais:

- Configuração do projeto: `.reimagined/skills/<name>/SKILL.md`
- Configuração global: `~/.config/reimagined/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o reimagined sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.reimagined/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/reimagined/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.reimagined/skills/git-release/SKILL.md` assim:
...
O reimagined lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `reimagined.json`:
...
**Para agentes embutidos** (em `reimagined.json`):
