---
title: Sunucu
description: reimagined sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`reimagined serve` komutu, reimagined istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `reimagined.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `OPENCODE_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `reimagined` değeridir; değiştirmek isterseniz `OPENCODE_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `reimagined serve` hem de `reimagined web` için geçerlidir.
...
`reimagined` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
reimagined ile programatik etkileşim için reimagined sunucusunu kullanın.
:::

Bu mimari, reimagined'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`reimagined serve` ile bağımsız bir sunucu başlatabilirsiniz. reimagined TUI açıksa, `reimagined serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum reimagined [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
reimagined sunucusu aşağıdaki API'leri sunar.
