...
describe("WSL server settings presentation", () => {
  ...
  test("offers install and update only when Reimagined needs attention", () => {
    ...
    ).toBe("Install Reimagined")
    expect(
      wslOpencodeAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/reimagined",
        version: "1.2.2",
        expectedVersion: "1.2.3",
        matchesDesktop: false,
        error: null,
      }),
    ).toBe("Update Reimagined")
    expect(
      wslOpencodeAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/reimagined",
  ...
  test("does not report ready when Reimagined is present but cannot run", () => {
    const model = addServerViewModel({
      state: {
        ...readyWslState,
        installed: [{ name: "Debian", version: 2, isDefault: true }],
        ...
        reimaginedChecks: {
          Debian: {
            distro: "Debian",
            resolvedPath: "/home/me/.reimagined/bin/reimagined",
            version: null,
            expectedVersion: "1.2.3",
            matchesDesktop: null,
    ...
    expect(model.primaryButton.action).toBe("install-reimagined")
  })
