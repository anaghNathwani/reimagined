...
reimagined ile ilgili bir sorunu ayiklamak icin once loglari ve diskte tuttugu yerel verileri kontrol edin.
...
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\reimagined\log` yapistirin

Log dosyalari zaman damgasiyla adlandirilir (ornegin `2025-01-09T123456.log`) ve en yeni 10 log dosyasi tutulur.

Daha ayrintili ayiklama bilgisi icin `--log-level` komut satiri secenegini kullanabilirsiniz. Ornek: `reimagined --log-level DEBUG`.

---

## Depolama

reimagined, oturum verilerini ve diger uygulama verilerini diskte su konumda saklar:

- **macOS/Linux**: `~/.local/share/reimagined/`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\reimagined` yapistirin
...
reimagined Desktop arka planda yerel bir reimagined sunucusu (`reimagined-cli` sidecar) calistirir. Sorunlarin cogu bozuk bir plugin, hasarli cache veya hatali sunucu ayarindan kaynaklanir.

### Hızlı kontroller

- Uygulamayi tamamen kapatip yeniden acin
- Uygulama hata ekrani gosteriyorsa **Restart**'a tiklayip hata detaylarini kopyalayin
- Yalnizca macOS: `reimagined` menusu -> **Reload Webview** (arayuz bos/donukse yardimci olur)
...
- **macOS/Linux**: `~/.config/reimagined/reimagined.jsonc` (veya `~/.config/reimagined/reimagined.json`)
- **macOS/Linux** (eski kurulumlar): `~/.local/share/reimagined/reimagined.jsonc`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\reimagined\reimagined.jsonc` yapistirin
...
reimagined diskten yerel plugin de yukleyebilir. Bu dizinleri gecici olarak tasiyin (veya klasoru yeniden adlandirin) ve uygulamayi yeniden baslatin:

- **Global plugins**
  - **macOS/Linux**: `~/.config/reimagined/plugins/`
  - **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\reimagined\plugins` yapistirin
...
Pluginleri kapatmak ise yaramazsa (veya plugin kurulumu takili kaldiysa), reimagined'un cache'i yeniden olusturmasi icin cache'i temizleyin.

1. reimagined Desktop'u tamamen kapatin
2. Cache dizinini silin:

- **macOS**: Finder -> `Cmd+Shift+G` -> `~/.cache/reimagined`
- **Linux**: `~/.cache/reimagined` dizinini silin (veya `rm -rf ~/.cache/reimagined` calistirin)
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.cache\reimagined` yapistirin

3. reimagined Desktop'u yeniden baslatin

---

### Sunucu bağlantı sorunlarını düzeltin

reimagined Desktop ya kendi yerel sunucusunu baslatir (varsayilan) ya da sizin tanimladiginiz bir sunucu URL'sine baglanir.
...
`reimagined.json(c)` dosyanizda `server` bolumu varsa gecici olarak kaldirin ve desktop uygulamasini yeniden baslatin.
...
Windows'ta reimagined Desktop, Microsoft Edge **WebView2 Runtime** gerektirir. Uygulama bos pencereyle aciliyorsa veya hic baslamiyorsa WebView2'yi kurup/guncelleyip tekrar deneyin.

---

### Windows: Genel performans sorunları

Windows'ta yavaslik, dosya erisim sorunlari veya terminal problemleri yasiyorsaniz [WSL (Windows Subsystem for Linux)](/docs/windows-wsl) kullanmayi deneyin. WSL, reimagined ozellikleriyle daha sorunsuz calisan bir Linux ortami saglar.

---

### Bildirimler görünmüyor

reimagined Desktop sistem bildirimlerini yalnizca su durumlarda gosterir:

- Isletim sistemi ayarlarinizda reimagined icin bildirimler etkinse
...
1. reimagined Desktop'u kapatin
2. Su dosyalari bulun ve silin (reimagined Desktop uygulama veri dizininde yer alirlar):

- `reimagined.settings.dat` (desktop varsayilan sunucu URL'si)
- `reimagined.global.dat` ve `reimagined.workspace.*.dat` (son sunucular/projeler gibi UI durumu)
...
reimagined ile ilgili bir sorun yasiyorsaniz:
...
Erisiminiz olan modelleri gormek icin `reimagined models` calistirin.
...
2. Sorun surerse kayitli konfigurasyonu temizlemeyi deneyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.local\share\reimagined`
...
API cagrisi hatalari aliyorsaniz bunun nedeni guncel olmayan provider paketleri olabilir. reimagined, provider paketlerini (OpenAI, Anthropic, Google vb.) gerektikce dinamik yukler ve yerelde onbellekler.
...
1. Provider paket cache'ini temizleyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.cache\reimagined`

2. En guncel provider paketlerini yeniden kurmak icin reimagined'u yeniden baslatin

Bu, reimagined'un en yeni provider paket surumlerini indirmesini zorlar ve model parametresi/API degisikliklerinden kaynakli uyumsuzluklari cogu zaman cozer.
...
reimagined, Wayland kullandiginizi algilarsa `wl-clipboard` tercihi yapar. Aksi halde sirayla `xclip` ve `xsel` araclarini arar.
