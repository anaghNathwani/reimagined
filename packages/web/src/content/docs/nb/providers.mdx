---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i Reimagined.
---

import config from "../../../../config.mjs"
export const console = config.console

Reimagined bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i Reimagined-konfigurasjonen.
...
i `~/.local/share/reimagined/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i Reimagined
konfig.
...
Reimagined Zen er en liste over modeller levert av Reimagined-teamet som har vært
testet og verifisert for å fungere godt med Reimagined. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med Reimagined Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `Reimagined Zen` og gå til [reimagined.ai/auth](https://reimagined.ai/zen).
...
Det fungerer som alle andre leverandører i Reimagined og er helt valgfritt å bruke.

---

## Reimagined Go

Reimagined Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av Reimagined-teamet som har vært
testet og verifisert for å fungere godt med Reimagined.

1. Kjør kommandoen `/connect` i TUI, velg `Reimagined Go`, og gå til [reimagined.ai/auth](https://reimagined.ai/zen).
...
Slik bruker du Amazon Bedrock med Reimagined:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører Reimagined:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `reimagined.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i reimagined støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere reimagined til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at Reimagined skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom Reimagined-konfigurasjonen.
...
for å lagre token i Reimagined auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
Reimagined bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse Reimagined
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`reimagined.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [reimagined-gitlab-auth](https://www.npmjs.com/package/reimagined-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `reimagined.json`:
...
Slik bruker du GitHub Copilot-abonnementet med Reimagined:
...
Slik bruker du Google Vertex AI med Reimagined:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører Reimagined.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom Reimagined, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [reimagined-helicone-session](https://github.com/H2Shami/reimagined-helicone-session) plugin for automatisk å logge hver Reimagined-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver Reimagined-samtale oppført som en separat økt.
...
Du kan konfigurere Reimagined for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere Reimagined for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere Reimagined for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for Reimagined. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/reimagined) for detaljer.
:::
...
Slik bruker du Ollama Cloud med Reimagined:
...
3. Kopier API-nøkkelen for bruk i Reimagined.
...
6. **Viktig**: Før du bruker skymodeller i Reimagined, må du hente modellinformasjonen lokalt:
...
Reimagined Zen er en liste over testede og verifiserte modeller levert av Reimagined-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>Reimagined Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **Reimagined Zen**.

   ```txt
   /connect
   ```

3. Skriv inn Reimagined API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom Reimagined-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom Reimagined-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Reimagined:
...
Du kan også tilpasse modeller gjennom Reimagined-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med Reimagined. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `reimagined.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar Reimagined forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `reimagined auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk Reimagined-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i Reimagined-konfigurasjonen.
