---
title: Plugins
description: Escreva seus próprios plugins para estender o reimagined.
---

Plugins permitem que você estenda o reimagined conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do reimagined.
...
- `.reimagined/plugins/` - Plugins em nível de projeto
- `~/.config/reimagined/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/reimagined/node_modules/`.
...
1. Configuração global (`~/.config/reimagined/reimagined.json`)
2. Configuração do projeto (`reimagined.json`)
3. Diretório de plugins global (`~/.config/reimagined/plugins/`)
4. Diretório de plugins do projeto (`.reimagined/plugins/`)
...
O reimagined executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do reimagined para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o reimagined.
...
:::note
Se você estiver usando o aplicativo desktop reimagined, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o reimagined de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao reimagined:
...
O helper `tool` cria uma ferramenta personalizada que o reimagined pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o reimagined junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://reimagined.ai/docs/sdk) para detalhes.
