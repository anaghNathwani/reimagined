---
title: CLI
description: Reimagined CLI Optionen und Befehle.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Der Reimagined CLI startet standardmäßig das [TUI](/docs/tui), wenn er ohne Argumente ausgeführt wird.

```bash
reimagined
```

Es akzeptiert aber auch Befehle, wie auf dieser Seite dokumentiert. Dadurch können Sie programmgesteuert mit Reimagined interagieren.
...
Starten Sie die Terminalbenutzeroberfläche Reimagined.
...
Der Reimagined CLI verfügt außerdem über die folgenden Befehle.

---

### agent

Agenten für Reimagined verwalten.
...
Schließen Sie ein Terminal an einen bereits laufenden Reimagined-Backend-Server an, der über die Befehle `serve` oder `web` gestartet wurde.

```bash
reimagined attach [url]
```

Dies ermöglicht die Verwendung von TUI mit einem Remote-Backend Reimagined. Zum Beispiel:
...
| <nobr><code>{"--username"}</code></nobr> | `-u` | Basic-Auth-Benutzername (standardmäßig `OPENCODE_SERVER_USERNAME` oder `reimagined`) |
...
Reimagined wird von der Anbieterliste bei [Models.dev](https://models.dev) unterstützt, sodass Sie `reimagined auth login` verwenden können, um API-Schlüssel für jeden Anbieter zu konfigurieren, den Sie verwenden möchten. Dies wird in `~/.local/share/reimagined/auth.json` gespeichert.

```bash
reimagined auth login
```

Wenn Reimagined startet, lädt es den Anbieter aus der Anmeldeinformationsdatei. Und ob in Ihren Umgebungen Schlüssel sind oder in Ihrem Projekt eine `.env`-Datei vorhanden ist.
...
Verwenden Sie das Flag `--refresh`, um die zwischengespeicherte Modellliste zu aktualisieren. Dies ist nützlich, wenn einem Anbieter neue Modelle hinzugefügt wurden und Sie diese in Reimagined sehen möchten.
...
Führen Sie Reimagined im nicht interaktiven Modus aus, indem Sie direkt eine Eingabeaufforderung übergeben.
...
Sie können auch eine Verbindung zu einer laufenden `reimagined serve`-Instanz herstellen, um bei jedem Lauf Kaltstartzeiten für den MCP-Server zu vermeiden:
...
| <nobr><code>{"--attach"}</code></nobr>   |      | An einen laufenden Reimagined-Server anschließen (z.B. http://localhost:4096)                         |
| <nobr><code>{"--password"}</code></nobr> | `-p` | Basic-Auth-Passwort (standardmäßig `OPENCODE_SERVER_PASSWORD`)                                      |
| <nobr><code>{"--username"}</code></nobr> | `-u` | Basic-Auth-Benutzername (standardmäßig `OPENCODE_SERVER_USERNAME` oder `reimagined`)                  |
...
Starten Sie einen Headless-Reimagined-Server für den API-Zugriff. Sehen Sie sich die [Server-Dokumentation](/docs/server) für die vollständige HTTP-Schnittstelle an.

```bash
reimagined serve
```

Dadurch wird ein HTTP-Server gestartet, der API-Zugriff auf Reimagined-Funktionalität ohne die TUI-Schnittstelle bietet. Legen Sie `OPENCODE_SERVER_PASSWORD` fest, um die HTTP-Basisauthentifizierung zu aktivieren (Benutzername ist standardmäßig `reimagined`).
...
Reimagined-Sitzungen verwalten.
...
Listen Sie alle Reimagined-Sitzungen auf.
...
Zeigen Sie Token-Nutzungs- und Kostenstatistiken für Ihre Reimagined-Sitzungen an.
...
Importieren Sie Sitzungsdaten aus einer JSON-Datei oder einer Reimagined-Freigabe-URL.

```bash
reimagined import <file>
```

Sie können aus einer lokalen Datei oder einer Reimagined-Freigabe-URL importieren.
...
Starten Sie einen Headless-Reimagined-Server mit einer Weboberfläche.

```bash
reimagined web
```

Dadurch wird ein HTTP-Server gestartet und ein Webbrowser geöffnet, um über eine Webschnittstelle auf Reimagined zuzugreifen. Legen Sie `OPENCODE_SERVER_PASSWORD` fest, um die HTTP-Basisauthentifizierung zu aktivieren (Benutzername ist standardmäßig `reimagined`).
...
Deinstallieren Sie Reimagined und entfernen Sie alle zugehörigen Dateien.
...
Aktualisiert Reimagined auf die neueste Version oder eine bestimmte Version.
...
Der Reimagined CLI akzeptiert die folgenden globalen Flags.
...
Reimagined kann mithilfe von Umgebungsvariablen konfiguriert werden.
...
| `OPENCODE_SERVER_USERNAME`            | Zeichenfolge    | Benutzernamen für die Basisauthentifizierung überschreiben (Standard `reimagined`) |
