...
reimagined'da iki tür agent vardır; birincil agent'lar ve alt agent'lar.
...
reimagined, **Build** ve **Plan** olmak üzere iki yerleşik birincil agent ile birlikte gelir. Bunlara aşağıda bakacağız.
...
Reimagined, **General**, **Explore** ve **Scout** olmak üzere üç yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

Reimagined iki yerleşik birincil agent ve üç yerleşik alt agent ile birlikte gelir.
...
Agent'ları `reimagined.json` yapılandırma dosyanızda yapılandırın:
...
- Proje başına: `.reimagined/agents/`
...
Sıcaklık belirtilmezse reimagined modeline özgü varsayılanları kullanır; çoğu model için genellikle 0, Qwen modelleri için 0,55.
...
Bu yol, yapılandırma dosyasının bulunduğu yere göredir. Yani bu hem global reimagined yapılandırması hem de projeye özel yapılandırma için işe yarar.
...
reimagined yapılandırmanızdaki model kimliği `provider/model-id` biçimini kullanır. Örneğin, [Reimagined Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `reimagined/gpt-5.1-codex` kullanırsınız.
...
:::tip
Mevcut modellerin listesini görmek için `reimagined models` komutunu çalıştırın.
:::
...
:::tip
Paylaşmak istediğiniz bir agent'ınız var mı? [Submit a PR](https://github.com/anomalyco/reimagined).
:::
