---
title: LSP Server
description: Reimagined lässt sich in Ihren LSP-Server integrieren.
---

Reimagined kann sich in Language Server Protocol (LSP)-Server integrieren, um Diagnosen als Feedback für den Agenten zu verwenden.

---

## Integriert

Reimagined verfügt über mehrere integrierte LSP-Server für gängige Sprachen:
...
Wenn LSP aktiviert ist und Reimagined eine Datei öffnet, geschieht Folgendes:
...
Sie können LSP-Server über den Abschnitt `lsp` in Ihrer Reimagined-Konfiguration aktivieren und anpassen.
...
```json title="reimagined.json" {5-7}
{
  "$schema": "https://reimagined.ai/config.json",
  "lsp": {
    "rust": {
...
```json title="reimagined.json" {5-9}
{
  "$schema": "https://reimagined.ai/config.json",
  "lsp": {
    "typescript": {
