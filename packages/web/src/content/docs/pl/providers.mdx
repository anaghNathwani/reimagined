---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w reimagined.
---

import config from "../../../../config.mjs"
export const console = config.console

reimagined używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji reimagined.
...
w `~/.local/share/reimagined/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim reimagined
config.
...
Reimagined Zen to lista modeli dostarczonych przez zespół reimagined, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z reimagined. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od Reimagined Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `Reimagined Zen` i przejdź do [reimagined.ai/auth](https://reimagined.ai/zen).
...
Działa jak każdy inny dostawca w reimagined i jest całkowicie opcjonalny w użyciu.

---

## Reimagined Go

Reimagined Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół reimagined, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z reimagined.

1. Uruchom polecenie `/connect` w TUI, wybierz `Reimagined Go` i przejdź do [reimagined.ai/auth](https://reimagined.ai/zen).
...
Aby używać Amazon Bedrock z reimagined:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania reimagined:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `reimagined.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w reimagined nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować reimagined do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod reimagined działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji reimagined.
...
reimagined używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować reimagined
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`reimagined.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [reimagined-gitlab-auth](https://www.npmjs.com/package/reimagined-gitlab-auth).
...
Aby korzystać z subskrypcji GitHub Copilot z kodem reimagined:
...
Aby używać Google Vertex AI z reimagined:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania reimagined.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą reimagined, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [reimagined-helicone-session](https://github.com/H2Shami/reimagined-helicone-session), aby automatycznie rejestrować każdą konwersację reimagined jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja reimagined będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować reimagined tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować reimagined tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować reimagined tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem reimagined. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/reimagined).
:::
...
Aby korzystać z Ollama Cloud z reimagined:
...
3. Skopiuj klucz API do użycia w reimagined.
...
6. **Ważne**: Przed użyciem modeli chmurowych w reimagined musisz pobrać informacje o modelu lokalnie:
...
Reimagined Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół reimagined. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>Reimagined Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **Reimagined Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API reimagined.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji reimagined.
...
5. Możesz je także dostosować za pomocą konfiguracji reimagined. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z reimagined:
...
Możesz także dostosować modele za pomocą konfiguracji reimagined. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z reimagined. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `reimagined.json` w katalogu projektu:
...
Pola `limit` pozwalają reimagined zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `reimagined auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację reimagined i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji reimagined.
