---
title: プラグイン
description: Reimagined を拡張する独自のプラグインを作成します。
---

プラグインを使用すると、さまざまなイベントにフックして動作をカスタマイズすることで Reimagined を拡張できます。プラグインを作成して、新しい機能を追加したり、外部サービスと統合したり、Reimagined のデフォルトの動作を変更したりできます。
...
- `.reimagined/plugins/` - プロジェクトレベルのプラグイン
- `~/.config/reimagined/plugins/` - グローバルプラグイン
...
**npm プラグイン** は、起動時に Bun を使用して自動的にインストールされます。パッケージとその依存関係は `~/.cache/reimagined/node_modules/` にキャッシュされます。
...
1. グローバル設定 (`~/.config/reimagined/reimagined.json`)
2. プロジェクト設定 (`reimagined.json`)
3. グローバルプラグインディレクトリ (`~/.config/reimagined/plugins/`)
4. プロジェクトプラグインディレクトリ (`.reimagined/plugins/`)
...
Reimagined は起動時に `bun install` を実行してこれらをインストールします。プラグインとツールはそれらをインポートできるようになります。
...
- `client`: AI と対話するための Reimagined SDK クライアント。
...
ここでは、Reimagined を拡張するために使用できるプラグインの例をいくつか示します。
...
レベル: `debug`、`info`、`warn`、`error`。詳細については、[SDK ドキュメント](https://reimagined.ai/docs/sdk) を参照してください。
...
Reimagined が `.env` ファイルを読み取らないようにします。
...
プラグインは Reimagined にカスタムツールを追加することもできます。
...
`tool` ヘルパーは、Reimagined が呼び出すことができるカスタムツールを作成します。 Zod スキーマ関数を受け取り、次のようなツール定義を返します。

- `description`: ツールの機能
- `args`: ツールの引数の Zod スキーマ
- `execute`: ツールが呼び出されたときに実行される関数

カスタムツールは、組み込みツールと並行して Reimagined で使用できます。
