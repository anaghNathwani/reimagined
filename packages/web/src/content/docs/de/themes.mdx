...
Mit Reimagined kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet Reimagined das `reimagined`-Theme.
...
Reimagined liefert mehrere eingebaute Themes mit.
...
- Reimagined optisch ans Terminal anpassen wollen
...
Reimagined unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/reimagined/themes/*.json` oder `$XDG_CONFIG_HOME/reimagined/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.reimagined/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.reimagined/themes/*.json`
