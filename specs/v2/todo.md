...
The reimagined server has moved to the Effect HttpApi backend. Remaining work is
...
- remove the public in-memory `@reimagined-ai/llm` tool loop after replacing its
...
- plugins get global "reimagined" instance like in that post i showed
- reimagined instance has stuff like `reimagined.session.prompt()` or
  `reimagined.tool.register({...})`
...
- decide whether to preserve deprecated `@reimagined-ai/llm` orchestration exports
