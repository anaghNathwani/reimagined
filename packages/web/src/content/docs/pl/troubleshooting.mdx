...
Aby debugować problemy z reimagined, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\reimagined\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `reimagined --log-level DEBUG`.

---

## Przechowywanie danych

reimagined przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/reimagined/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\reimagined`
...
reimagined Desktop uruchamia w tle lokalny serwer reimagined (sidecar `reimagined-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `Reimagined` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/reimagined/reimagined.jsonc` (lub `~/.config/reimagined/reimagined.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/reimagined/reimagined.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\reimagined\reimagined.jsonc`
...
reimagined może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/reimagined/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\reimagined\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby reimagined mógł ją odbudować.

1. Całkowicie zamknij reimagined Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/reimagined`
- **Linux**: usuń `~/.cache/reimagined` (lub uruchom `rm -rf ~/.cache/reimagined`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\reimagined`

3. Uruchom ponownie reimagined Desktop.

---

### Napraw problemy z połączeniem z serwerem

reimagined Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `reimagined.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.
...
W systemie Windows reimagined Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami reimagined.

---

### Brak powiadomień

reimagined Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla reimagined w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie reimagined Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji reimagined Desktop):

- `reimagined.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `reimagined.global.dat` i `reimagined.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
Jeśli masz problemy z reimagined:
...
3. Upewnij się, że masz najnowszą wersję z `reimagined upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `reimagined models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\reimagined`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. reimagined dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\reimagined`

2. Uruchom ponownie kod reimagined, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to reimagined do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
reimagined wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
