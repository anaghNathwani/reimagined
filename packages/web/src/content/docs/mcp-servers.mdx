...
You can add external tools to Reimagined using the _Model Context Protocol_, or MCP. Reimagined supports both local and remote servers.
...
You can define MCP servers in your [Reimagined Config](https://reimagined.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/reimagined` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
Reimagined automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, Reimagined will:
...
If the server requires authentication, Reimagined will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `reimagined mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, Reimagined will store the tokens securely in `~/.local/share/reimagined/mcp-auth.json`.
...
Your MCPs are available as tools in Reimagined, alongside built-in tools. So you can manage them through the Reimagined config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect Reimagined to your Sentry account.
