...
Reimagined Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

Go, Reimagined'daki diğer sağlayıcılar gibi çalışır. Reimagined Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve Reimagined'u kullanmak için bunu kullanmanıza gerek yoktur.
...
Reimagined Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

Reimagined Go, Reimagined'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>Reimagined Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `Reimagined Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye Reimagined Go'ya abone olabilir.
:::
...
Reimagined Go aşağıdaki limitleri içerir:
...
Reimagined yapılandırmanızdaki [model id](/docs/config/#models) formatı `reimagined-go/<model-id>` şeklindedir. Örneğin, Kimi K3 için yapılandırmanızda `reimagined-go/kimi-k3` kullanmalısınız.
...
Reimagined Go'yu şu amaçlarla oluşturduk:
...
4. Reimagined ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
