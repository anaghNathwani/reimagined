...
Le skill degli agenti permettono a Reimagined di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
Reimagined cerca in queste posizioni:

- Config di progetto: `.reimagined/skills/<name>/SKILL.md`
- Config globale: `~/.config/reimagined/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, Reimagined risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.reimagined/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/reimagined/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.reimagined/skills/git-release/SKILL.md` cosi':
...
Reimagined elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `reimagined.json`:
...
**Per agenti integrati** (in `reimagined.json`):
