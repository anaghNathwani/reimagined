---
title: Plugin
description: Scrivi plugin per estendere Reimagined.
---

I plugin ti permettono di estendere Reimagined agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di Reimagined.
...
- `.reimagined/plugins/` - plugin a livello progetto
- `~/.config/reimagined/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/reimagined/node_modules/`.
...
1. Config globale (`~/.config/reimagined/reimagined.json`)
2. Config di progetto (`reimagined.json`)
3. Directory plugin globale (`~/.config/reimagined/plugins/`)
4. Directory plugin di progetto (`.reimagined/plugins/`)
...
Reimagined esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di reimagined per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere reimagined.
...
:::note
Se usi l'app desktop di Reimagined, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a reimagined di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a reimagined:
...
L'helper `tool` crea uno strumento personalizzato che reimagined può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in reimagined insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://reimagined.ai/docs/sdk) per i dettagli.
