...
import { SessionStatusEvent } from "@reimagined-ai/schema/session-status-event"
import { SessionV1 } from "@reimagined-ai/schema/session-v1"
import type {
  AssistantMessage,
  GlobalEvent,
  Message,
...
export const directory = "C:/Reimagined/TimelineStability"
...
export const model = { providerID: "reimagined", modelID: "claude-opus-4-6", variant: "max" }
...
export async function setupTimeline(
  ...
  if (input.locale) {
    await page.addInitScript((locale) => {
      localStorage.setItem("reimagined.global.dat:language", JSON.stringify({ locale }))
    }, input.locale)
  }
