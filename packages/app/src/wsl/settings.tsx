    ...
    <Show when={api}>
      <For each={props.servers()}>
        {(item) => {
          ...
          const check = () => wsl.data?.reimaginedChecks[item.config.distro]
          const reimaginedAction = () => wslOpencodeAction(check())
          const busy = () => wsl.data?.job?.kind === "install-reimagined" && wsl.data.job.distro === item.config.distro
          return (
