---
title: Anbieter
description: Verwendung eines beliebigen LLM-Anbieters in Reimagined.
---

import config from "../../../../config.mjs"
export const console = config.console

Reimagined verwendet [AI SDK](https://ai-sdk.dev/) und [Models.dev](https://models.dev) zur Unterstützung von **75+ LLM-Anbietern** und unterstützt die Ausführung lokaler Modelle.

Um einen Anbieter hinzuzufügen, müssen Sie:

1. Fügen Sie die API-Schlüssel für den Anbieter mit dem Befehl `/connect` hinzu.
2. Konfigurieren Sie den Anbieter in Ihrer Reimagined-Konfiguration.
...
Sie können die Anbieter über den Abschnitt `provider` in Ihrem Reimagined anpassen
config.
...
Reimagined Zen ist eine Liste von Modellen, die vom Reimagined-Team bereitgestellt wurden
Getestet und verifiziert, dass es gut mit Reimagined funktioniert. [Learn more](/docs/zen).

:::tip
Wenn Sie neu sind, empfehlen wir Ihnen, mit Reimagined Zen zu beginnen.
:::

1. Führen Sie den Befehl `/connect` in TUI aus, wählen Sie Reimagined aus und gehen Sie zu [reimagined.ai/auth](https://reimagined.ai/auth).
...
Es funktioniert wie jeder andere Anbieter in Reimagined und ist völlig optional.

---

## Reimagined Go

Reimagined Go ist ein kostenguenstiges Abonnement, das zuverlaessigen Zugriff auf beliebte Open-Coding-Modelle bietet, die vom Reimagined-Team getestet und verifiziert wurden, dass sie gut mit Reimagined funktionieren.

1. Führen Sie den Befehl `/connect` in der TUI aus, waehlen Sie `Reimagined Go` und gehen Sie zu [reimagined.ai/auth](https://reimagined.ai/zen).
...
So verwenden Sie Amazon Bedrock mit Reimagined:
...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Legen Sie eine dieser Umgebungsvariablen fest, während Sie Reimagined ausführen:
   ...
   Für eine projektspezifische oder dauerhafte Konfiguration verwenden Sie `reimagined.json`:
...
:::info
Die Verwendung Ihres Claude Pro/Max-Abonnements in Reimagined wird von [Anthropic](https://anthropic.com) nicht offiziell unterstützt.
:::
...
Sie können reimagined so konfigurieren, dass es lokale Modelle über [Atomic Chat](https://atomic.chat) verwendet — eine Desktop-Anwendung, die lokale LLMs hinter einem OpenAI-kompatiblen API-Server bereitstellt (Standard-Endpunkt `http://127.0.0.1:1337/v1`).
...
2. Gehen Sie zu [Azure AI Foundry](https://ai.azure.com/) und stellen Sie ein Modell bereit.
   ...
   :::note
   Der Bereitstellungsname muss mit dem Modellnamen übereinstimmen, damit Reimagined ordnungsgemäß funktioniert.
   :::
...
5. Führen Sie den Befehl `/models` aus, um ein Modell auszuwählen.
   ...
   Sie können Modelle auch über Ihre Reimagined-Konfiguration hinzufügen.
...
2. Wählen Sie Ihre Authentifizierungsmethode:
   ...
   3. Name: `Reimagined`, Bereiche: `api`
...
um das Token im Reimagined-Authentifizierungsspeicher zu speichern.
:::

##### Selbstgehostetes GitLab

:::note[Compliance-Hinweis]
Reimagined verwendet ein kleines Modell für einige AI-Aufgaben wie das Generieren des Sitzungstitels.
Es ist standardmäßig für die Verwendung von gpt-5-nano konfiguriert und wird von Zen gehostet. Zum Sperren von Reimagined
Um nur Ihre eigene von GitLab gehostete Instanz zu verwenden, fügen Sie Folgendes zu Ihrer hinzu
`reimagined.json`-Datei. Es wird außerdem empfohlen, die Sitzungsfreigabe zu deaktivieren.
...
Weitere Dokumentation auf der [reimagined-gitlab-auth](https://www.npmjs.com/package/reimagined-gitlab-auth)-Homepage.

##### Konfiguration

Anpassen über `reimagined.json`:
...
So verwenden Sie Ihr GitHub Copilot-Abonnement mit Reimagined:
...
So verwenden Sie Google Vertex AI mit Reimagined:
...
2. Legen Sie die erforderlichen Umgebungsvariablen fest:
   ...
   Legen Sie sie fest, während Sie Reimagined ausführen.
...
Falls Sie eine Funktion oder ein Modell von Helicone sehen, das nicht automatisch über Reimagined konfiguriert wird, können Sie es jederzeit selbst konfigurieren.
...
Das Plugin fügt `Helicone-Session-Id`- und `Helicone-Session-Name`-Header in Ihre Anfragen ein. Auf der Seite „Sitzungen“ von Helicone wird jede Reimagined-Konversation als separate Sitzung aufgeführt.
...
Sie können Reimagined für die Verwendung lokaler Modelle über das Dienstprogramm [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server konfigurieren
...
Sie können Reimagined über LM Studio für die Verwendung lokaler Modelle konfigurieren.
...
Sie können Reimagined über Ollama für die Verwendung lokaler Modelle konfigurieren.

:::tip
Ollama kann sich automatisch für Reimagined konfigurieren. Weitere Informationen finden Sie unter [Ollama integration docs](https://docs.ollama.com/integrations/reimagined).
:::
...
So verwenden Sie Ollama Cloud mit Reimagined:
...
3. Kopieren Sie den API-Schlüssel zur Verwendung in Reimagined.
...
6. **Wichtig**: Bevor Sie Cloud-Modelle in Reimagined verwenden, müssen Sie die Modellinformationen lokal abrufen:
...
Reimagined Zen ist eine Liste getesteter und verifizierter Modelle, die vom Reimagined-Team bereitgestellt werden. [Learn more](/docs/zen).

1. Melden Sie sich bei **<a href={console}>Reimagined Zen</a>** an und klicken Sie auf **Create API Key**.

2. Führen Sie den Befehl `/connect` aus und suchen Sie nach **Reimagined Zen**.

   ```txt
   /connect
   ```

3. Geben Sie Ihren Reimagined API-Schlüssel ein.
...
4. Viele OpenRouter-Modelle sind standardmäßig vorinstalliert. Führen Sie den Befehl `/models` aus, um das gewünschte Modell auszuwählen.
   ...
   Sie können auch zusätzliche Modelle über Ihre Reimagined-Konfiguration hinzufügen.
...
5. Sie können sie auch über Ihre Reimagined-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe eines Anbieters
...
So verwenden Sie [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) mit Reimagined:
...
Sie können Modelle auch über Ihre Reimagined-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe der Provider-Routing-Reihenfolge.
...
:::tip
Sie können jeden OpenAI-kompatiblen Anbieter mit Reimagined verwenden. Die meisten modernen AI-Anbieter bieten OpenAI-kompatible APIs an.
:::
...
4. Erstellen oder aktualisieren Sie Ihre `reimagined.json`-Datei in Ihrem Projektverzeichnis:
...
Mithilfe der `limit`-Felder kann Reimagined erkennen, wie viel Kontext Sie noch haben. Standardanbieter beziehen diese automatisch von models.dev.
...
1. **Überprüfen Sie die Authentifizierungseinrichtung**: Führen Sie `reimagined auth list` aus, um zu sehen, ob die Anmeldeinformationen vorhanden sind
...
2. Überprüfen Sie bei benutzerdefinierten Anbietern die Reimagined-Konfiguration und:
   - Stellen Sie sicher, dass der im Befehl `/connect` verwendete Anbieter ID mit dem ID in Ihrer Reimagined-Konfiguration übereinstimmt.
