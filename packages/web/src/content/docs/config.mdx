---
title: Config
description: Using the Reimagined JSON config.
---

You can configure Reimagined using a JSON config file.

---

## Format

Reimagined supports both **JSON** and **JSONC** (JSON with Comments) formats.

```jsonc title="reimagined.jsonc"
{
...
1. **Remote config** (from `.well-known/reimagined`) - organizational defaults
2. **Global config** (`~/.config/reimagined/reimagined.json`) - user preferences
3. **Custom config** (`OPENCODE_CONFIG` env var) - custom overrides
4. **Project config** (`reimagined.json` in project) - project-specific settings
5. **`.reimagined` directories** - agents, commands, plugins
6. **Inline config** (`OPENCODE_CONFIG_CONTENT` env var) - runtime overrides
7. **Managed config files** (`/Library/Application Support/reimagined/` on macOS) - admin-controlled
...
:::note
The `.reimagined` and `~/.config/reimagined` directories use **plural names** for subdirectories: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, and `themes/`. Singular names (e.g., `agent/`) are also supported for backwards compatibility.
:::
...
Organizations can provide default configuration via the `.well-known/reimagined` endpoint. This is fetched automatically when you authenticate with a provider that supports it.
...
```json title="Remote config from .well-known/reimagined"
{
  "mcp": {
    "jira": {
      "type": "remote",
...
Place your global Reimagined config in `~/.config/reimagined/reimagined.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.

For TUI-specific settings, use `~/.config/reimagined/tui.json`.
...
Add `reimagined.json` in your project root. Project config has the highest precedence among standard config files - it overrides both global and remote configs.
...
When Reimagined starts up, it first looks for a config file in the current directory, then traverses up to the nearest Git directory.
...
modes, and plugins just like the standard `.reimagined` directory, and should
...
The custom directory is loaded after the global config and `.reimagined` directories, so it **can override** their settings.
...
Drop an `reimagined.json` or `reimagined.jsonc` file in the system managed config directory:

| Platform | Path                                     |
| -------- | ---------------------------------------- |
| macOS    | `/Library/Application Support/reimagined/` |
| Linux    | `/etc/reimagined/`                         |
| Windows  | `%ProgramData%\reimagined`                 |

These directories require admin/root access to write, so users cannot modify them.

#### macOS managed preferences

On macOS, Reimagined reads managed preferences from the `ai.reimagined.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.

Reimagined checks these paths:

1. `/Library/Managed Preferences/<user>/ai.reimagined.managed.plist`
2. `/Library/Managed Preferences/ai.reimagined.managed.plist`

The plist keys map directly to `reimagined.json` fields. MDM metadata keys (`PayloadUUID`, `PayloadType`, etc.) are stripped automatically.

**Creating a `.mobileconfig`**

Use the `ai.reimagined.managed` PayloadType. The Reimagined config keys go directly in the payload dict:
...
<dict>
  ...
  <array>
    <dict>
      <key>PayloadType</key>
      <string>ai.reimagined.managed</string>
      <key>PayloadIdentifier</key>
      <string>com.example.reimagined.config</string>
      <key>PayloadUUID</key>
  ...
  <key>PayloadIdentifier</key>
  <string>com.example.reimagined</string>
  <key>PayloadUUID</key>
  <string>GENERATE-YOUR-OWN-UUID</string>
  <key>PayloadVersion</key>
  <integer>1</integer>
...
The server/runtime config schema is defined in [**`reimagined.ai/config.json`**](https://reimagined.ai/config.json).

TUI config uses [**`reimagined.ai/tui.json`**](https://reimagined.ai/tui.json).
...
Legacy `theme`, `keybinds`, and `tui` keys in `reimagined.json` are deprecated and automatically migrated when possible.

---

### Server

You can configure server settings for the `reimagined serve` and `reimagined web` commands through the `server` option.
...
- `mdns` - Enable mDNS service discovery. This allows other devices on the network to discover your Reimagined server.
- `mdnsDomain` - Custom domain name for mDNS service. Defaults to `reimagined.local`. Useful for running multiple instances on the same network.
...
If not specified, Reimagined will automatically discover and use a sensible default based on your operating system (e.g. `pwsh` or `cmd.exe` on Windows, `/bin/zsh` or `/bin/bash` on macOS/Linux). You can provide an absolute path or a short name.
...
You can configure the providers and models you want to use in your Reimagined config through the `provider`, `model` and `small_model` options.
...
The `small_model` option configures a separate model for lightweight tasks like title generation. By default, Reimagined tries to use a cheaper model if one is available from your provider, otherwise it falls back to your main model.
...
Use the `experimental.policies` option to allow or deny Reimagined actions on configured resources. Currently, policies can control which providers Reimagined may use.
...
Reimagined normalizes image attachments before sending them to the model. By default, images are resized when they exceed `2000x2000` pixels or `5242880` base64 bytes.
...
If an image still cannot fit after resizing, Reimagined omits oversized tool-result images or fails oversized user-provided images with an image size error.
...
You can also define agents using markdown files in `~/.config/reimagined/agents/` or `.reimagined/agents/`. [Learn more here](/docs/agents).
...
The default agent must be a primary agent (not a subagent). This can be a built-in agent like `"build"` or `"plan"`, or a [custom agent](/docs/agents) you've defined. If the specified agent doesn't exist or is a subagent, Reimagined will fall back to `"build"` with a warning.

This setting applies across all interfaces: TUI, CLI (`reimagined run`), desktop app, and GitHub Action.
...
You can also define commands using markdown files in `~/.config/reimagined/commands/` or `.reimagined/commands/`. [Learn more here](/docs/commands).
...
Reimagined uses snapshots to track file changes during agent operations, enabling you to undo and revert changes within a session. Snapshots are enabled by default.
...
Reimagined will automatically download any new updates when it starts up. You can disable this with the `autoupdate` option.
...
By default, reimagined **allows all operations** without requiring explicit approval. You can change this using the `permission` option.
...
[Plugins](/docs/plugins) extend Reimagined with custom tools, hooks, and integrations.

Place plugin files in `.reimagined/plugins/` or `~/.config/reimagined/plugins/`. You can also load plugins from npm through the `plugin` option.

```json title="reimagined.json"
{
  "$schema": "https://reimagined.ai/config.json",
  "plugin": ["reimagined-helicone-session", "@my-org/custom-plugin"]
}
```
...
This is useful when you want to restrict Reimagined to only use specific providers rather than disabling them one by one.
