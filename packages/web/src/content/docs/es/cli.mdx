---
title: CLI
description: Opciones y comandos de la CLI de Reimagined.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI de Reimagined de forma predeterminada inicia el [TUI](/docs/tui) cuando se ejecuta sin ningún argumento.

```bash
reimagined
```

Pero también acepta comandos como se documenta en esta página. Esto le permite interactuar con Reimagined mediante programación.
...
Inicie la interfaz de usuario del terminal Reimagined.
...
La CLI de Reimagined también tiene los siguientes comandos.

---

### agent

Administrar agentes para Reimagined.
...
Conecte una terminal a un servidor backend Reimagined que ya se esté ejecutando y iniciado mediante los comandos `serve` o `web`.

```bash
reimagined attach [url]
```

Esto permite usar TUI con un backend remoto Reimagined. Por ejemplo:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuario de autenticación básica (predeterminado: `OPENCODE_SERVER_USERNAME` u `reimagined`) |
...
Reimagined funciona con la lista de proveedores en [Models.dev](https://models.dev), por lo que puede usar `reimagined auth login` para configurar las claves API para cualquier proveedor que desee utilizar. Esto se almacena en `~/.local/share/reimagined/auth.json`.

```bash
reimagined auth login
```

Cuando se inicia Reimagined, carga los proveedores desde el archivo de credenciales. Y si hay claves definidas en sus entornos o un archivo `.env` en su proyecto.
...
Utilice el indicador `--refresh` para actualizar la lista de modelos almacenados en caché. Esto es útil cuando se han agregado nuevos modelos a un proveedor y desea verlos en Reimagined.
...
Ejecute reimagined en modo no interactivo pasando un mensaje directamente.
...
También puede conectarse a una instancia `reimagined serve` en ejecución para evitar tiempos de arranque en frío del servidor MCP en cada ejecución:
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Adjuntar a un servidor reimagined en ejecución (por ejemplo, http://localhost:4096)         |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Contraseña de autenticación básica (predeterminada: `OPENCODE_SERVER_PASSWORD`)           |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuario de autenticación básica (predeterminado: `OPENCODE_SERVER_USERNAME` u `reimagined`) |
...
Inicie un servidor Reimagined sin cabeza para acceso API. Consulte los [documentos del servidor](/docs/server) para conocer la interfaz HTTP completa.

```bash
reimagined serve
```

Esto inicia un servidor HTTP que proporciona acceso API a la funcionalidad reimagined sin la interfaz TUI. Configure `OPENCODE_SERVER_PASSWORD` para habilitar la autenticación básica HTTP (el nombre de usuario predeterminado es `reimagined`).
...
Administrar Reimagined sesiones.
...
Enumere todas las sesiones Reimagined.
...
Muestre el uso de tokens y las estadísticas de costos para sus sesiones Reimagined.
...
Importe datos de sesión desde un archivo JSON o una URL compartida Reimagined.

```bash
reimagined import <file>
```

Puede importar desde un archivo local o una URL compartida Reimagined.
...
Inicie un servidor Reimagined sin cabeza con una interfaz web.

```bash
reimagined web
```

Esto inicia un servidor HTTP y abre un navegador web para acceder a Reimagined a través de una interfaz web. Configure `OPENCODE_SERVER_PASSWORD` para habilitar la autenticación básica HTTP (el nombre de usuario predeterminado es `reimagined`).
...
Desinstale Reimagined y elimine todos los archivos relacionados.
...
Actualiza reimagined a la última versión o a una versión específica.
...
La CLI de Reimagined toma las siguientes banderas globales.
...
Reimagined se puede configurar mediante variables de entorno.
...
| `OPENCODE_SERVER_USERNAME`            | cadena   | Anular el nombre de usuario de autenticación básica (predeterminado `reimagined`) |
