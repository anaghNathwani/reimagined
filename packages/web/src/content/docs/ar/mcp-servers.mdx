...
يمكنك إضافة أدوات خارجية إلى Reimagined باستخدام _Model Context Protocol_، أو MCP. يدعم Reimagined خوادم محلية وبعيدة.
...
يمكنك تعريف خوادم MCP في [إعدادات Reimagined](https://reimagined.ai/docs/config/) ضمن `mcp`. أضف كل خادم MCP باسم فريد. ويمكنك الإشارة إلى ذلك الخادم بالاسم عند توجيه الطلب إلى الـ LLM.
...
يمكن للمنظمات توفير خوادم MCP الافتراضية عبر نقطة النهاية `.well-known/reimagined` الخاصة بها. قد تكون هذه الخوادم معطّلة افتراضيا، مما يتيح للمستخدمين تفعيل ما يحتاجونه فقط.
...
يتولى Reimagined تلقائيا معالجة مصادقة OAuth لخوادم MCP البعيدة. عندما يتطلب خادم ما المصادقة، سيقوم Reimagined بما يلي:
...
إذا كان الخادم يتطلب المصادقة، فسيطلب منك Reimagined المصادقة عند أول محاولة لاستخدامه. وإذا لم يحدث ذلك، يمكنك [تشغيل التدفق يدويا](#authenticating) باستخدام `reimagined mcp auth <server-name>`.
...
سيفتح أمر `mcp auth` متصفحك للتفويض. بعد إكمال التفويض، سيخزّن Reimagined الرموز بشكل آمن في `~/.local/share/reimagined/mcp-auth.json`.
...
تتوفر خوادم MCP لديك كأدوات داخل Reimagined إلى جانب الأدوات المضمنة. لذا يمكنك إدارتها عبر إعدادات Reimagined مثل أي أداة أخرى.
...
سيؤدي ذلك إلى فتح نافذة متصفح لإكمال تدفق OAuth وربط Reimagined بحساب Sentry الخاص بك.
