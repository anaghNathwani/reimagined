...
U Reimagined mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, Reimagined koristi nasu `reimagined` temu.
...
Reimagined dolazi sa vise ugradenih tema.
...
- Zele da Reimagined odgovara izgledu njihovog terminala
...
Reimagined podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/reimagined/themes/*.json` ili `$XDG_CONFIG_HOME/reimagined/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.reimagined/themes/*.json`
4. **Trenutni radni direktorij** - `./.reimagined/themes/*.json`
