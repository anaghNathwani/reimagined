...
import { and, Database, eq, isNull, lt, or, sql } from "@reimagined-ai/console-core/drizzle/index.js"
import { KeyTable } from "@reimagined-ai/console-core/schema/key.sql.js"
import { BillingTable, LiteTable, SubscriptionTable, UsageTable } from "@reimagined-ai/console-core/schema/billing.sql.js"
import { centsToMicroCents } from "@reimagined-ai/console-core/util/price.js"
import { getMonthlyBounds, getWeekBounds } from "@reimagined-ai/console-core/util/date.js"
import { Identifier } from "@reimagined-ai/console-core/identifier.js"
import { Billing } from "@reimagined-ai/console-core/billing.js"
import { Actor } from "@reimagined-ai/console-core/actor.js"
import { WorkspaceTable } from "@reimagined-ai/console-core/schema/workspace.sql.js"
import { ZenData } from "@reimagined-ai/console-core/model.js"
import { Subscription } from "@reimagined-ai/console-core/subscription.js"
import { BlackData } from "@reimagined-ai/console-core/black.js"
import { UserTable } from "@reimagined-ai/console-core/schema/user.sql.js"
import { ModelTable } from "@reimagined-ai/console-core/schema/model.sql.js"
import { ProviderTable } from "@reimagined-ai/console-core/schema/provider.sql.js"
...
import { LiteData } from "@reimagined-ai/console-core/lite.js"
import { Resource } from "@reimagined-ai/console-resource"
...
import { Workspace } from "@reimagined-ai/console-core/workspace.js"
...
export async function handler(
  ...
  try {
    ...
    const sessionId = input.request.headers.get("x-reimagined-session") ?? ""
    const requestId = input.request.headers.get("x-reimagined-request") ?? ""
    const ocClient = input.request.headers.get("x-reimagined-client") ?? ""
    const projectId = input.request.headers.get("x-reimagined-project") ?? ""
      ...
      if (!allowedRegions?.includes("unavailable"))
        throw new RegionError(
          t("zen.api.error.regionNotAllowed", {
            consoleGoUrl: `https://reimagined.ai/workspace/${authInfo.workspaceID}/go`,
          }),
        )
    }
    */
    ...
    const retriableRequest = async (retry: RetryOptions = { excludeProviders: [], retryCount: 0 }) => {
      ...
      const res = await fetchWithRetryableStatus(
        reqUrl,
        {
          ...
          headers: (() => {
            ...
            headers.delete("x-reimagined-request")
            headers.delete("x-reimagined-session")
            headers.delete("x-reimagined-project")
            headers.delete("x-reimagined-client")
            return headers
          })(),
          body: reqBody,
      ...
      if (isNewInference) {
        const resEndpointId = res.headers.get("x-reimagined-endpoint-id")
        const resEndpointModelId = res.headers.get("x-reimagined-upstream-model-id")
  ...
  function validateBilling(authInfo: AuthInfo, modelInfo: ModelInfo): BillingSource {
    ...
    if (opts.modelList === "lite" && authInfo.billing.lite && authInfo.lite) {
      try {
        const consoleGoUrl = `https://reimagined.ai/workspace/${authInfo.workspaceID}/go`
    ...
    const billingUrl = `https://reimagined.ai/workspace/${authInfo.workspaceID}/billing`
    const membersUrl = `https://reimagined.ai/workspace/${authInfo.workspaceID}/members`
