---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i Reimagined.
---

import config from "../../../../config.mjs"
export const console = config.console

Reimagined bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i Reimagined-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/reimagined/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i Reimagined-konfigurationen.
...
Reimagined Zen er en liste over modeller leveret af Reimagined-teamet, som er blevet testet og verificeret til at fungere godt med Reimagined. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med Reimagined Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg reimagined og gå til [reimagined.ai/auth](https://reimagined.ai/auth).
...
Det fungerer som alle andre udbydere i Reimagined og er helt valgfrit at bruge.

---

## Reimagined Go

Reimagined Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af Reimagined-teamet, som er testet og verificeret til at fungere godt med Reimagined.

1. Kør kommandoen `/connect` i TUI, vælg `Reimagined Go`, og gå til [reimagined.ai/auth](https://reimagined.ai/zen).
...
Sådan bruger du Amazon Bedrock med Reimagined:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører reimagined:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `reimagined.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i Reimagined understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere reimagined til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at reimagined skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem reimagined-konfigurationen.
...
gemme tokenet i reimagined auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
Reimagined bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse Reimagined
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`reimagined.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [reimagined-gitlab-auth](https://www.npmjs.com/package/reimagined-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `reimagined.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med Reimagined:
...
Sådan bruger du Google Vertex AI med Reimagined:
...
2. Angiv de nødvendige miljøvariabler:
   ...
   Sæt dem mens du kører reimagined.
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem reimagined, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [reimagined-helicone-session](https://github.com/H2Shami/reimagined-helicone-session) plugin for automatisk at logge hver Reimagined-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver Reimagined-samtale opført som en separat session.
...
Du kan konfigurere reimagined til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere reimagined til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere reimagined til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for Reimagined. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/reimagined) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med Reimagined:
...
3. Kopier API-nøglen til brug i Reimagined.
...
6. **Vigtigt**: Før du bruger skymodeller i Reimagined, skal du hente modelinformationen lokalt:
...
Reimagined Zen er en liste over testede og verificerede modeller leveret af Reimagined-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>Reimagined Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **Reimagined Zen**.

   ```txt
   /connect
   ```

3. Indtast Reimagined API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem reimagined-konfigurationen.
...
5. Du kan også tilpasse dem gennem reimagined-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Reimagined:
...
Du kan også tilpasse modeller gennem reimagined-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med reimagined. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `reimagined.json`-filen i projektkataloget:
...
`limit`-felterne lader Reimagined forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `reimagined auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek reimagined-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i reimagined-konfigurationen.
