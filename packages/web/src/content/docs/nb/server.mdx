---
title: Tjener
description: Samhandle med Reimagined-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `reimagined serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en Reimagined-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `reimagined.local` |
...
Sett `OPENCODE_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `reimagined` som standard, eller sett `OPENCODE_SERVER_USERNAME` for å overstyre det. Dette gjelder både `reimagined serve` og `reimagined web`.
...
Når du kjører `reimagined` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk Reimagined-serveren til å samhandle med Reimagined programmatisk.
:::

Denne arkitekturen lar Reimagined støtte flere klienter og lar deg samhandle med Reimagined programmatisk.

Du kan kjøre `reimagined serve` for å starte en frittstående server. Hvis du har
Reimagined TUI kjører, vil `reimagined serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av Reimagined [IDE](/docs/ide) plugins.
...
reimagined-serveren viser følgende APIer.
