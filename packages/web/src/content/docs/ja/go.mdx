...
Reimagined Goは低価格のサブスクリプションで、**初月は5ドル**、その後は**月額10ドル**で、人気のオープンなコーディングモデルに安定してアクセスできます。

GoはReimaginedの他のプロバイダーと同様に機能します。Reimagined GoをサブスクライブしてAPIキーを取得します。これは**完全に任意**であり、Reimaginedを使用するために必須ではありません。
...
:::tip
Reimaginedでうまく動作する一部のモデルとプロバイダーをテストしました。
:::
...
Reimagined Goを使用すると、これらのモデルに**初月は5ドル**、その後は**月額10ドル**でアクセスできます。

---

## 仕組み

Reimagined Goは、Reimaginedの他のプロバイダーと同様に機能します。

1. **<a href={console}>Reimagined Zen</a>**にサインインし、GoをサブスクライブしてAPIキーをコピーします。
2. TUIで`/connect`コマンドを実行し、`Reimagined Go`を選択して、APIキーを貼り付けます。
3. TUIで`/models`を実行すると、Goを通じて利用可能なモデルのリストが表示されます。

:::note
Reimagined Goをサブスクライブできるのは、1つのワークスペースにつき1メンバーのみです。
:::
...
Reimagined設定の[model id](/docs/config/#models)は、`reimagined-go/<model-id>`という形式を使用します。たとえば、Kimi K3の場合は、設定で`reimagined-go/kimi-k3`を使用します。
...
4. Reimaginedで他のプロバイダーも使用できるようにすることで、**ベンダーロックインをなくす**。
