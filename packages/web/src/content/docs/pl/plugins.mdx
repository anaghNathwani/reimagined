---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć reimagined.
---

Wtyczki umożliwiające wyłączenie reimagined poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania reimagined.
...
- `.reimagined/plugins/` - Wtyczki na poziomie projektu
- `~/.config/reimagined/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/reimagined/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/reimagined/reimagined.json`)
2. Konfiguracja projektu (`reimagined.json`)
3. Globalny katalog wtyczek (`~/.config/reimagined/plugins/`)
4. Katalog wtyczek projektu (`.reimagined/plugins/`)
...
reimagined uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z reimagined do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do reimagined.
...
:::note
Jeśli korzystasz z aplikacji komputerowej reimagined, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj reimagined czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do reimagined:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod reimagined. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://reimagined.ai/docs/sdk).
