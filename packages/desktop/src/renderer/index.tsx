...
import {
  ...
  useCommand,
  useWslServers,
} from "@reimagined-ai/app"
import type { UpdaterState } from "@reimagined-ai/app/updater"
...
const deepLinkEvent = "reimagined:deep-link"
...
function windowLastActiveUrlKey(windowID: string) {
  return `reimagined.desktop.window.${windowID}.last-active-url`
}
  ...
  return {
    ...
    notify: async (title, description, href) => {
      ...
      const notification = new Notification(title, {
        body: description ?? "",
        icon: "https://reimagined.ai/favicon-96x96-v3.png",
      })
...
function DesktopRoot(props: { windowState: DesktopWindowState }) {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("reimagined.global.dat").getItem("language")
