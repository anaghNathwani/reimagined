...
import { createOpencode, type ToolPart } from "@reimagined-ai/sdk"
...
console.log("🚀 Starting reimagined server...")
const reimagined = await createOpencode({
  port: 0,
})
...
void (async () => {
  const events = await reimagined.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new reimagined session...")
    const { client, server } = reimagined
    ...
    console.log("✅ Created reimagined session:", createResult.data.id)
  ...
  console.log("📝 Sending to reimagined:", message.text)
