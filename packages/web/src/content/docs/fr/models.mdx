...
Les fournisseurs les plus populaires sont préchargés par défaut. Si vous avez ajouté les informations d'identification d'un fournisseur via la commande `/connect`, elles seront disponibles lorsque vous démarrerez Reimagined.
...
Voici plusieurs modèles qui fonctionnent bien avec Reimagined, sans ordre particulier. (Cette liste n’est pas exhaustive et n’est pas nécessairement à jour) :
...
Pour définir l'un d'entre eux comme modèle par défaut, vous pouvez définir la clé `model` dans votre configuration Reimagined.

```json title="reimagined.json" {3}
{
...
Ici, l'identifiant complet est `provider_id/model_id`. Par exemple, si vous utilisez [Reimagined Zen](/docs/zen), vous utiliserez `reimagined/gpt-5.1-codex` pour GPT 5.1 Codex.
...
```jsonc title="reimagined.jsonc" {7-12,19-24}
{
  "$schema": "https://reimagined.ai/config.json",
  "provider": {
    "openai": {
...
```jsonc title="reimagined.jsonc" {6-21}
{
  "$schema": "https://reimagined.ai/config.json",
  "provider": {
    "reimagined": {
...
De nombreux modèles prennent en charge plusieurs variantes avec différentes configurations. Reimagined est livré avec des variantes par défaut intégrées pour les fournisseurs populaires.

### Variantes intégrées

Reimagined est livré avec des variantes par défaut pour de nombreux fournisseurs :
...
```jsonc title="reimagined.jsonc" {7-18}
{
  "$schema": "https://reimagined.ai/config.json",
  "provider": {
    "openai": {
...
Lorsque Reimagined démarre, il recherche les modèles dans l'ordre de priorité suivant :
...
2. La liste des modèles dans la configuration Reimagined.
