---
title: Eklentiler
description: Reimagined'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek Reimagined'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya Reimagined'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.reimagined/plugins/` - Proje düzeyinde eklentiler
- `~/.config/reimagined/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/reimagined/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/reimagined/reimagined.json`)
2. Project config (`reimagined.json`)
3. Global eklenti dizini (`~/.config/reimagined/plugins/`)
4. Proje eklenti dizini (`.reimagined/plugins/`)
...
Reimagined bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir Reimagined SDK istemcisi.
...
Reimagined'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
Reimagined masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
Reimagined'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca Reimagined'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, Reimagined'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra Reimagined'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://reimagined.ai/docs/sdk) bakın.
