---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u Reimagined.
---

import config from "../../../../config.mjs"
export const console = config.console

Reimagined koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj Reimagined konfiguraciji.
...
u `~/.local/share/reimagined/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem Reimagined
config.
...
Reimagined Zen je lista modela koje je obezbedio Reimagined tim koji su bili
testirano i potvrđeno da dobro radi sa Reimagined. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa Reimagined Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite reimagined i idite na [reimagined.ai/auth](https://reimagined.ai/auth).
...
Radi kao i svaki drugi provajder u Reimagined i potpuno je opcionalan za korištenje.

---

## Reimagined Go

Reimagined Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža Reimagined tim i koji su testirani i verificirani da dobro rade s Reimagined-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `Reimagined Go` i idite na [reimagined.ai/auth](https://reimagined.ai/zen).
...
Da biste koristili Amazon Bedrock s Reimagined:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete reimagined:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `reimagined.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u Reimagined.
:::
...
Možete konfigurirati reimagined za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi reimagined ispravno radio.
   :::
...
Također možete dodati modele kroz svoju reimagined konfiguraciju.
...
da pohrani token u reimagined auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
Reimagined koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate Reimagined
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`reimagined.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [reimagined-gitlab-auth](https://www.npmjs.com/package/reimagined-gitlab-auth).

##### Konfiguracija

Prilagodite putem `reimagined.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s reimagined:
...
Za korištenje Google Vertex AI s Reimagined:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće reimagined.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem reimagineda, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [reimagined-helicone-session](https://github.com/H2Shami/reimagined-helicone-session) da automatski prijavite svaki Reimagined razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki Reimagined razgovor naveden kao zasebna sesija.
...
Možete konfigurirati reimagined za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati reimagined za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati reimagined za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za Reimagined. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/reimagined) za detalje.
:::
...
Da biste koristili Ollama Cloud s Reimagined:
...
3. Kopirajte API ključ za korištenje u Reimagined.
...
6. **Važno**: Prije upotrebe modela oblaka u Reimagined, morate lokalno povući informacije o modelu:
...
Reimagined Zen je lista testiranih i verifikovanih modela koju je obezbedio Reimagined tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>Reimagined Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **Reimagined Zen**.

```txt
   /connect
```

3. Unesite svoj Reimagined API ključ.
...
Također možete dodati dodatne modele putem vaše reimagined konfiguracije.
...
5. Također ih možete prilagoditi putem vaše reimagined konfiguracije. Evo primjera navođenja provajdera
...
Također možete prilagoditi modele kroz svoju reimagined konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s reimagined-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `reimagined.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju Reimagined da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `reimagined auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite Reimagined konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj reimagined konfiguraciji.
