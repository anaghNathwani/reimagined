import type { ProviderAuthAuthorization, ProviderAuthMethod } from "@reimagined-ai/sdk/v2/client"
import { Button } from "@reimagined-ai/ui/button"
import { useDialog } from "@reimagined-ai/ui/context/dialog"
import { Dialog } from "@reimagined-ai/ui/dialog"
import { Icon } from "@reimagined-ai/ui/icon"
import { IconButton } from "@reimagined-ai/ui/icon-button"
import { List, type ListRef } from "@reimagined-ai/ui/list"
import { ProviderIcon } from "@reimagined-ai/ui/provider-icon"
import { Spinner } from "@reimagined-ai/ui/spinner"
import { Tag } from "@reimagined-ai/ui/tag"
import { TextField } from "@reimagined-ai/ui/text-field"
import { ButtonV2 } from "@reimagined-ai/ui/v2/button-v2"
import { DialogBody, DialogHeader, DialogTitle, DialogV2 } from "@reimagined-ai/ui/v2/dialog-v2"
import { TextInputV2 } from "@reimagined-ai/ui/v2/text-input-v2"
...
function ProviderPicker(props: {
  ...
  const note = (id: string) => {
    ...
    if (id === "reimagined-go") return language.t("dialog.provider.reimaginedGo.tagline")
    return undefined
  }
    ...
    >
      {(i) => (
        <div class="px-1.25 w-full flex items-center gap-x-3">
          ...
          <span>{i.name}</span>
          <Show when={i.id === "reimagined"}>
            <div class="text-14-regular text-text-weak">{language.t("dialog.provider.reimagined.tagline")}</div>
          </Show>
...
function ProviderPickerV2(props: {
  ...
  const featured = ["reimagined", "reimagined-go", "anthropic", "openai", "google", "openrouter", "vercel"]
                  ...
                  <For each={group.items()}>
                    {(provider) => (
                      ...
                      >
                        ...
                        <Show when={provider.id === "reimagined" || provider.id === "reimagined-go"}>
                          <span class="min-w-0 truncate font-[440] text-v2-text-text-muted">
                            {language.t(
                              provider.id === "reimagined"
                                ? "dialog.provider.reimagined.tagline"
                                : "dialog.provider.reimaginedGo.tagline",
                            )}
                          </span>
  ...
  function ApiAuthView() {
    ...
    if (newLayout())
      return (
        <div class="flex flex-col gap-5 px-3 text-[13px] font-[440] leading-5 tracking-[-0.04px] text-v2-text-text-muted">
          <Show
            when={provider().id === "reimagined"}
            fallback={language.t("provider.connect.apiKey.description", { provider: provider().name })}
          >
            <div class="flex flex-col gap-5">
              <div>{language.t("provider.connect.reimaginedZen.line1")}</div>
              <div>{language.t("provider.connect.reimaginedZen.line2")}</div>
              <div>
    ...
    return (
      <div class="flex flex-col gap-6">
        <Switch>
          <Match when={provider().id === "reimagined"}>
            <div class="flex flex-col gap-4">
              <div class="text-14-regular text-text-base">{language.t("provider.connect.reimaginedZen.line1")}</div>
