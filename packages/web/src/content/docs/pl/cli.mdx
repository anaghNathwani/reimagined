---
title: CLI
description: Opcje i polecenia wiersza poleceń Reimagined.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

CLI Reimagined domyślnie uruchamia [TUI](/docs/tui), gdy wywołasz je bez argumentów.

```bash
reimagined
```

CLI obsługuje też polecenia opisane na tej stronie, więc możesz używać Reimagined programowo.
...
Uruchamia interfejs terminalowy (TUI) Reimagined.
...
Interfejs CLI Reimagined zawiera także dodatkowe polecenia.

---

### agent

Zarządzaj agentami Reimagined.
...
Dołącz terminal do już działającego serwera backendu Reimagined uruchomionego za pomocą `serve` lub `web`.

```bash
reimagined attach [url]
```

Pozwala to połączyć TUI ze zdalnym backendem Reimagined. Na przykład:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nazwa użytkownika uwierzytelniania podstawowego (domyślnie `OPENCODE_SERVER_USERNAME` lub `reimagined`) |
...
Reimagined integruje się z wieloma dostawcami. Możesz użyć `reimagined auth login`, aby skonfigurować klucz API dla dowolnego obsługiwanego dostawcy. Klucze są bezpiecznie przechowywane w `~/.local/share/reimagined/auth.json`.

```bash
reimagined auth login
```

Podczas uruchamiania Reimagined ładuje dostawców z pliku poświadczeń, a także ze zmiennych środowiskowych lub pliku `.env` w Twoim projekcie.
...
Użyj flagi `--refresh`, aby zaktualizować listę modeli w pamięci podręcznej. Jest to przydatne, gdy dostawca dodał nowe modele, które chcesz zobaczyć w Reimagined.
...
Uruchom Reimagined w trybie nieinteraktywnym, przekazując bezpośrednio prompt.
...
Można również użyć tego z działającym `reimagined serve`, aby uniknąć zimnego startu serwera MCP przy każdym uruchomieniu:
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Dołącz do działającego serwera Reimagined (np. http://localhost:4096)                                   |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Hasło uwierzytelniania podstawowego (domyślnie `OPENCODE_SERVER_PASSWORD`)                            |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nazwa użytkownika uwierzytelniania podstawowego (domyślnie `OPENCODE_SERVER_USERNAME` lub `reimagined`) |
...
Uruchom serwer Reimagined (bez interfejsu) w celu uzyskania dostępu do API. Pełny opis API HTTP znajduje się w [dokumentacji serwera](/docs/server).

```bash
reimagined serve
```

Uruchamia to serwer HTTP, który zapewnia dostęp do API Reimagined bez interfejsu TUI. Ustaw `OPENCODE_SERVER_PASSWORD`, aby włączyć podstawowe uwierzytelnianie HTTP (domyślna nazwa użytkownika to `reimagined`).
...
Lista wszystkich sesji Reimagined.
...
Pokaż statystyki wykorzystania tokenów i sesji Reimagined.
...
Importuj dane sesji z pliku JSON lub adresu URL udostępniania Reimagined.
...
Uruchom serwer Reimagined z interfejsem internetowym.

```bash
reimagined web
```

Uruchamia to serwer HTTP i udostępnia Reimagined przez interfejs przeglądarkowy. Ustaw `OPENCODE_SERVER_PASSWORD`, aby włączyć podstawowe uwierzytelnianie HTTP (domyślna nazwa użytkownika to `reimagined`).
...
Odinstaluj Reimagined i usuń wszystkie powiązane pliki.
...
Aktualizuj Reimagined do najnowszej lub określonej wersji.
...
Interfejs CLI Reimagined przyjmuje następujące flagi globalne dla każdego polecenia.
...
Reimagined można skonfigurować za pomocą zmiennych środowiskowych.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Nazwa użytkownika do autoryzacji (domyślnie `reimagined`)    |
