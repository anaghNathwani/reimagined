...
Dzięki reimagined możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie reimagined używa naszego własnego motywu `reimagined`.
...
reimagined ma kilka wbudowanych motywów.
...
- Chcą, aby Reimagined pasował do wyglądu ich terminala
...
reimagined obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/reimagined/themes/*.json` lub `$XDG_CONFIG_HOME/reimagined/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.reimagined/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.reimagined/themes/*.json`
