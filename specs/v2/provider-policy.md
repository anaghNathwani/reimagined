...
Both `action` and `resource` use reimagined's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.reimagined` files is intentionally deferred until `.reimagined` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether reimagined permits an operation through its managed execution paths.
