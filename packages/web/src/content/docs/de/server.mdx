---
title: Server
description: Nutze den reimagined-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `reimagined serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein reimagined-Client nutzen kann.
...
Der Benutzername ist standardmaessig `reimagined`, kann aber mit `OPENCODE_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `reimagined serve` und `reimagined web`.
...
Wenn du `reimagined` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den reimagined-Server, um reimagined programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `reimagined serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `reimagined serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die Reimagined-[IDE](/docs/ide)-Plugins.
...
Der reimagined-Server stellt folgende APIs bereit.
