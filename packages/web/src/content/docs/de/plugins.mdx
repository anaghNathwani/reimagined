---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um Reimagined zu erweitern.
---

Mit Plugins können Sie Reimagined erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von Reimagined zu ändern.
...
- `.reimagined/plugins/` – Plugins auf Projektebene
- `~/.config/reimagined/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/reimagined/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/reimagined/reimagined.json`)
2. Projektkonfiguration (`reimagined.json`)
3. Globales Plugin-Verzeichnis (`~/.config/reimagined/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.reimagined/plugins/`)
...
Reimagined führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein Reimagined-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie Reimagined erweitern können.
...
:::note
Wenn Sie die Reimagined-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass Reimagined `.env`-Dateien liest:
...
Plugins können Reimagined auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das Reimagined aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für Reimagined zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://reimagined.ai/docs/sdk).
