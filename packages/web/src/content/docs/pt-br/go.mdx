...
O Reimagined Go é uma assinatura de baixo custo — **US$ 5 no seu primeiro mês**, depois **US$ 10/mês** — que oferece acesso confiável a modelos abertos de programação populares.

O Go funciona como qualquer outro provedor no Reimagined. Você assina o Reimagined Go e
...
O Reimagined Go lhe dá acesso a esses modelos por **US$ 5 no seu primeiro mês**, depois **US$ 10/mês**.

---

## Como funciona

O Reimagined Go funciona como qualquer outro provedor no Reimagined.

1. Você entra no **<a href={console}>Reimagined Zen</a>**, assina o Go e
   copia a sua chave de API.
2. Você executa o comando `/connect` na TUI, seleciona `Reimagined Go` e cola
   a sua chave de API.
3. Execute `/models` na TUI para ver a lista de modelos disponíveis através do Go.

:::note
Apenas um membro por workspace pode assinar o Reimagined Go.
:::
...
O Reimagined Go inclui os seguintes limites:
...
O [ID do modelo](/docs/config/#models) na sua configuração do Reimagined
usa o formato `reimagined-go/<model-id>`. Por exemplo, para o Kimi K3, você usaria
`reimagined-go/kimi-k3` na sua configuração.
...
4. Não ter **nenhuma restrição de fornecedor (lock-in)**, permitindo que você use qualquer outro provedor com o Reimagined também.
