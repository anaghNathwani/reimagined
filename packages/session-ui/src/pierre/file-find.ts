...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("reimagined-find")
  api.delete("reimagined-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("reimagined-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("reimagined-find", new Highlight(...rest))
    return true
  }
