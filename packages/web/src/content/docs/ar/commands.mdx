...
أنشئ الملف `.reimagined/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات Reimagined أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ Reimagined:
...
- على مستوى النظام: `~/.config/reimagined/commands/`
- لكل مشروع: `.reimagined/commands/`
...
يتضمن reimagined عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
