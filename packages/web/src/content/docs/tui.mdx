---
title: TUI
description: Using the Reimagined terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Reimagined provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running Reimagined starts the TUI for the current directory.
...
When using the Reimagined TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to Reimagined. Allows you to select from available providers and add their API keys.
...
Exit Reimagined. _Aliases_: `/quit`, `/q`
...
This is separate from `reimagined.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long Reimagined waits after the leader key. Defaults to `2000`.
...
- `sound_pack` - Sound pack ID to use. Defaults to `reimagined.default`.
