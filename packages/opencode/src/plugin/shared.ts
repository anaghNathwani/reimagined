...
export const DEPRECATED_PLUGIN_PACKAGES = ["reimagined-openai-codex-auth", "reimagined-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, reimaginedVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(reimaginedVersion) || semver.major(reimaginedVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.reimagined
  if (typeof range !== "string") return
  if (!semver.satisfies(reimaginedVersion, range)) {
    throw new Error(`Plugin requires reimagined ${range} but running ${reimaginedVersion}`)
  }
}
