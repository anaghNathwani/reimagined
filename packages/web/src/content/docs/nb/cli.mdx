---
title: CLI
description: Reimagined CLI-alternativer og kommandoer.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Reimagined CLI starter som standard [TUI](/docs/tui) når den kjøres uten noen argumenter.

```bash
reimagined
```

Men den godtar også kommandoer som dokumentert på denne siden. Dette lar deg samhandle med Reimagined programmatisk.
...
Start Reimagined-terminalbrukergrensesnittet.
...
Administrer agenter for Reimagined.
...
Koble en terminal til en allerede kjørende Reimagined-backend-server startet via `serve` eller `web` kommandoer.

```bash
reimagined attach [url]
```

Dette gjør det mulig å bruke TUI med en ekstern Reimagined-backend. For eksempel:
...
| <nobr><code>{"--username"}</code></nobr> | `-u` | Brukernavn for grunnleggende autentisering (standard: `OPENCODE_SERVER_USERNAME` eller `reimagined`) |
...
Reimagined drives av leverandørlisten på [Models.dev](https://models.dev), så du kan bruke `reimagined auth login` til å konfigurere API-nøkler for enhver leverandør du vil bruke. Dette er lagret i `~/.local/share/reimagined/auth.json`.

```bash
reimagined auth login
```

Når Reimagined starter opp, laster den leverandørene fra filen med påloggingsinformasjon. Og hvis det er noen nøkler definert i miljøene dine eller en `.env`-fil i prosjektet ditt.
...
Bruk `--refresh`-flagget for å oppdatere den bufrede modelllisten. Dette er nyttig når nye modeller er lagt til en leverandør og du vil se dem i Reimagined.
...
Kjør Reimagined i ikke-interaktiv modus ved å sende en melding direkte.
...
Du kan også koble til en kjørende `reimagined serve`-forekomst for å unngå MCP-serveres kaldstarttid ved hver kjøring:
...
| <nobr><code>{"--attach"}</code></nobr>   |      | Koble til en kjørende Reimagined-server (f.eks. http://localhost:4096)                               |
| <nobr><code>{"--password"}</code></nobr> | `-p` | Passord for grunnleggende autentisering (standard: `OPENCODE_SERVER_PASSWORD`)                     |
| <nobr><code>{"--username"}</code></nobr> | `-u` | Brukernavn for grunnleggende autentisering (standard: `OPENCODE_SERVER_USERNAME` eller `reimagined`) |
...
Start en headless Reimagined-server for API-tilgang. Sjekk ut [server-dokumentene](/docs/server) for hele HTTP-grensesnittet.

```bash
reimagined serve
```

Dette starter en HTTP-server som gir API tilgang til Reimagined-funksjonalitet uten TUI-grensesnittet. Sett `OPENCODE_SERVER_PASSWORD` for å aktivere HTTP grunnleggende autentisering (brukernavn er standard til `reimagined`).
...
List opp alle Reimagined-økter.
...
Vis tokenbruk og kostnadsstatistikk for Reimagined-øktene dine.
...
Importer øktdata fra en JSON-fil eller en Reimagined share-URL.
...
Start en headless Reimagined-server med et webgrensesnitt.

```bash
reimagined web
```

Dette starter en HTTP-server og åpner en nettleser for å få tilgang til Reimagined via et nettgrensesnitt. Sett `OPENCODE_SERVER_PASSWORD` for å aktivere HTTP grunnleggende autentisering (brukernavn er standard til `reimagined`).
...
Avinstaller Reimagined og fjern alle relaterte filer.
...
Oppdaterer Reimagined til siste versjon eller en spesifikk versjon.
...
Reimagined CLI bruker følgende globale flagg.
...
Reimagined kan konfigureres ved hjelp av miljøvariabler.
...
| `OPENCODE_SERVER_USERNAME`            | streng | Overstyr grunnleggende autentiseringsbrukernavn (standard `reimagined`) |
