...
import { ReimaginedHttpApi } from "./api"
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for reimagined serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(ReimaginedHttpApi)
...
export const PublicApi = ReimaginedHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "reimagined",
    version: "1.0.0",
    description: "reimagined api",
