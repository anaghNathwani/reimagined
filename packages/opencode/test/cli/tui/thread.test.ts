...
describe("tui thread", () => {
  ...
  cliIt.live("rejects mini-only options without --mini", ({ reimagined }) =>
    Effect.gen(function* () {
      const result = yield* reimagined.spawn(["--replay-limit", "10"])

      reimagined.expectExit(result, 1)
      expect(result.stderr).toContain("--replay-limit requires --mini")
    }),
  )

  cliIt.live("routes attached sessions to mini mode", ({ reimagined }) =>
    Effect.gen(function* () {
      const result = yield* reimagined.spawn(["attach", "http://127.0.0.1:1", "--mini"])
  ...
  cliIt.live("rejects network options in mini mode", ({ reimagined }) =>
    Effect.gen(function* () {
      const result = yield* reimagined.spawn(["--mini", "--port", "4096"])
