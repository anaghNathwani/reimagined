...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@reimagined-ai/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { AppNodeBuilder } from "@reimagined-ai/core/effect/app-node-builder"
import { errorData, errorMessage } from "@reimagined-ai/tui/util/error"
...
import { hasTheme, upsertTheme } from "@reimagined-ai/tui/context/theme"
import { Global } from "@reimagined-ai/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@reimagined-ai/core/util/flock"
import { Flag } from "@reimagined-ai/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@reimagined-ai/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@reimagined-ai/core/v1/config/plugin"
import { createCommandShim } from "@reimagined-ai/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@reimagined-ai/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".reimagined"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".reimagined", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".reimagined", "tui.json"),
  }
}
