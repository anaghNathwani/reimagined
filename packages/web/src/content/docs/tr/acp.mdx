---
title: ACP Desteği
description: reimagined'u ACP uyumlu herhangi bir editörde kullanın.
---

reimagined, [Agent Client Protocol](https://agentclientprotocol.com) veya (ACP) dosyalarını destekleyerek onu doğrudan uyumlu editörlerde ve IDE'lerde kullanmanızı sağlar.
...
reimagined'u ACP aracılığıyla kullanmak için düzenleyicinizi `reimagined acp` komutunu çalıştıracak şekilde yapılandırın.

Komut, reimagined'u, editörünüzle stdio aracılığıyla JSON-RPC üzerinden iletişim kuran ACP uyumlu bir alt süreç olarak başlatır.
...
Açmak için AI Chat agent seçicisindeki yeni 'Reimagined' ajanını kullanın.
...
reimagined'u [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim)'de ACP ajanı olarak kullanmak için Neovim yapılandırmanıza aşağıdakileri ekleyin:
...
Bu yapılandırma, CodeCompanion'ı reimagined'u sohbet için ACP ajanı olarak kullanacak şekilde ayarlar.
...
reimagined, terminalde olduğu gibi ACP aracılığıyla aynı şekilde çalışır. Tüm özellikler desteklenir:
...
- reimagined yapılandırmanızda yapılandırılmış MCP sunucuları
