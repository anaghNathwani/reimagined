...
description: Skriv dine egne programtillegg for å utvide Reimagined.
---

Plugins lar deg utvide Reimagined ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til Reimagined.
...
- `.reimagined/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/reimagined/node_modules/`.
...
1. Global konfigurasjon (`~/.config/reimagined/reimagined.json`)
2. Prosjektkonfigurasjon (`reimagined.json`)
3. Global plugin-katalog (`~/.config/reimagined/plugins/`)
4. Prosjektpluginkatalog (`.reimagined/plugins/`)
...
Reimagined kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En Reimagined SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide Reimagined.
...
:::note
Hvis du bruker Reimagined-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre Reimagined fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til Reimagined:
...
`tool`-hjelperen lager et tilpasset verktøy som Reimagined kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for Reimagined sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://reimagined.ai/docs/sdk) for detaljer.
