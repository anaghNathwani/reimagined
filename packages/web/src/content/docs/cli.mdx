---
title: CLI
description: Reimagined CLI options and commands.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

The Reimagined CLI by default starts the [TUI](/docs/tui) when run without any arguments.

```bash
reimagined
```

But it also accepts commands as documented on this page. This allows you to interact with Reimagined programmatically.
...
Start the Reimagined terminal user interface.
...
The Reimagined CLI also has the following commands.

---

### agent

Manage agents for Reimagined.
...
| Flag                                        | Short | Description                                                                                                                                                                                                                |
| ------------------------------------------- | ----- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| <nobr><code>{"--path"}</code></nobr>        |       | Directory to write the agent file to (defaults to global or `.reimagined/agent` based on the prompt)                                                                                                                         |
...
Attach a terminal to an already running Reimagined backend server started via `serve` or `web` commands.

```bash
reimagined attach [url]
```

This allows using the TUI with a remote Reimagined backend. For example:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Basic auth username (defaults to `OPENCODE_SERVER_USERNAME` or `reimagined`) |
...
Reimagined is powered by the provider list at [Models.dev](https://models.dev), so you can use `reimagined auth login` to configure API keys for any provider you'd like to use. This is stored in `~/.local/share/reimagined/auth.json`.

```bash
reimagined auth login
```

When Reimagined starts up it loads the providers from the credentials file. And if there are any keys defined in your environments or a `.env` file in your project.
...
Use the `--refresh` flag to update the cached model list. This is useful when new models have been added to a provider and you want to see them in Reimagined.
...
Run reimagined in non-interactive mode by passing a prompt directly.
...
You can also attach to a running `reimagined serve` instance to avoid MCP server cold boot times on every run:
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Attach to a running reimagined server (e.g., http://localhost:4096)          |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Basic auth password (defaults to `OPENCODE_SERVER_PASSWORD`)               |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Basic auth username (defaults to `OPENCODE_SERVER_USERNAME` or `reimagined`) |
...
Start a headless Reimagined server for API access. Check out the [server docs](/docs/server) for the full HTTP interface.

```bash
reimagined serve
```

This starts an HTTP server that provides API access to reimagined functionality without the TUI interface. Set `OPENCODE_SERVER_PASSWORD` to enable HTTP basic auth (username defaults to `reimagined`).
...
Delete an Reimagined session.

```bash
reimagined session delete <sessionID>
```
...
Show token usage and cost statistics for your Reimagined sessions.
...
Import session data from a JSON file or Reimagined share URL.

```bash
reimagined import <file>
```

You can import from a local file or an Reimagined share URL.
...
Start a headless Reimagined server with a web interface.

```bash
reimagined web
```

This starts an HTTP server and opens a web browser to access Reimagined through a web interface. Set `OPENCODE_SERVER_PASSWORD` to enable HTTP basic auth (username defaults to `reimagined`).
...
```bash
reimagined plugin <module>
```

Or use the alias.

```bash
reimagined plug <module>
```
...
Fetch and checkout a GitHub PR branch, then run Reimagined.

```bash
reimagined pr <number>
```
...
Uninstall Reimagined and remove all related files.
...
Updates reimagined to the latest version or a specific version.
...
The reimagined CLI takes the following global flags.
...
Reimagined can be configured using environment variables.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Override basic auth username (default `reimagined`) |
